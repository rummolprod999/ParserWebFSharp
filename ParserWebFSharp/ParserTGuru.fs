...
type ParserTGuru(stn: Settings.T) =
    ...
    override this.Parsing() =
        for i in 1..pageC do
            try
                let url =
                    sprintf "https://www.tenderguru.ru/kommercheskie_tendery?region=&cat=&page=%d" i
    ...
    member private this.ParsingPage(url: string) =
        let Page =
            Download.DownloadString1251Cookies url
        ...
        | s ->
            ...
            let tens =
                documents.QuerySelectorAll("div.navigation_filter + table > tbody > tr")
    ...
    member private this.ParsingTenderAngle (t: IElement) (url: string) =
        ...
        | x when not (x.StartsWith("https://www.tenderguru.ru/")) ->
            raise
            <| NullReferenceException(sprintf "Href not contains https://www.tenderguru.ru/ in %s" url)
        | _ -> ()
        ...
        let dateEnd =
            datePub.AddDays(float endDateT)
        ...
        try
            let T =
                TenderTGuru(set, ten, 100, "TenderGURU", "http://www.tenderguru.ru")
