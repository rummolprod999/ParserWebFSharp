...
type TenderVolgZmo(stn: Settings.T, tn: VolgZmoRec, typeFz: int, etpName: string, etpUrl: string) =
    ...
    override this.Parsing() =
        ...
        let res =
                   builder {
                        ...
                        let IdOrg = ref 0
                        let orgName = InlineHtmlNavigator nav "//th[contains(., 'Организация, осуществляющая закупку:')]/following-sibling::td"
                        ...
                        if orgName <> "" then
                            ...
                            | false ->
                                ...
                                let contactPerson = ""
                                let postAddress = InlineHtmlNavigator nav "//th[contains(., 'Почтовый адрес')]/following-sibling::td"
                                let factAddress = InlineHtmlNavigator nav "//th[contains(., 'Место нахождения:')]/following-sibling::td"
                                let phone = InlineHtmlNavigator nav "//th[contains(., 'Номер контактного телефона')]/following-sibling::td"
                                let inn = ""
                                let email = InlineHtmlNavigator nav "//th[contains(., 'Адрес электронной почты:')]/following-sibling::td"
    ...
    member private this.GetLots(con: MySqlConnection, idTender: int, doc: HtmlDocument) =
        ...
        let currency = InlineHtmlNavigator nav "//th[contains(., 'Валюта:')]/following-sibling::td"
        let finSource = InlineHtmlNavigator nav "//th[contains(., 'Источник финансирования:')]/following-sibling::td"
        ...
        let purObjs = doc.DocumentNode.SelectNodes("//tr[contains(., 'Единица измерения')]//following-sibling::tr[contains(@class, 'portlet-section-body results-row')]")
        match purObjs with
        | null -> ()
        | _ -> 
        ...
        let delivPlace = InlineHtmlNavigator nav "//th[contains(., 'Место доставки товара, выполнения работы или оказания услуги:')]/following-sibling::td"
        let delivTerm = InlineHtmlNavigator nav "//th[contains(., 'Сроки поставки товара или завершения работы либо график оказания услуг:')]/following-sibling::td"
