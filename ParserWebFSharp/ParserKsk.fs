...
type ParserKsk(stn: Settings.T) =
    ...
    let urls =
        [| "http://www.gt-ksk.com/about/tenders/" |]
    ...
    member private __.ParsingPage(url: string) =
        ...
        | s ->
            ...
            let tens =
                documents
                    .QuerySelectorAll("div.tenders__block.tenders-block")
                    .ToList()
    ...
    member private __.ParsingTender (t: IElement) (url: string) =
        ...
        let res =
            builder {
                ...
                let! purName =
                    t.GsnDocWithError "div.tenders-block__text"
                    <| sprintf "purName not found %s %s " url (t.TextContent)

                let! purNum =
                    t.GsnDocWithError "div.tenders-block__status-num"
                    <| sprintf "purNum not found %s %s " url (t.TextContent)

                let purNum = purNum.Replace("№", "").Trim()

                let purNum =
                    sprintf "%s-%s" purNum (Tools.createMD5 purName)
                ...
                let! dates =
                    t.GsnDocWithError "div.tenders-block__date"
                ...
                if dateStartT <> "" then
                    ...
                    let! datePub =
                        dateStartT.DateFromStringDoc("dd.MM.yyyy", sprintf "datePub not found %s %s " href dateStartT)
                ...
                let T =
                    TenderKsk(set, tend, 246, "АО \"КСК\"", "http://www.gt-ksk.com/")
