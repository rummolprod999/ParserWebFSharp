...
type TenderGmt(stn: Settings.T, tn: GmtRec, typeFz: int, etpName: string, etpUrl: string) =
    ...
    override this.Parsing() =
        ...
        let res =
            builder {
                ...
                let orgName =
                    InlineHtmlNavigator nav "//td[. = 'Наименование:']/following-sibling::td"
                ...
                if orgName <> "" then
                    ...
                    | false ->
                        ...
                        let contactPerson =
                            InlineHtmlNavigator nav "//td[. = 'Контактное лицо:']/following-sibling::td"

                        let postAddress =
                            InlineHtmlNavigator nav "//td[. = 'Место нахождения:']/following-sibling::td"

                        let factAddress = ""

                        let phone =
                            InlineHtmlNavigator nav "//td[. = 'Телефон:']/following-sibling::td"

                        let inn = ""

                        let email =
                            InlineHtmlNavigator nav "//td[. = 'Адрес электронной почты:']/following-sibling::td"
    ...
    member private this.GetLots(con: MySqlConnection, idTender: int, doc: HtmlDocument) =
        ...
        let currency =
            InlineHtmlNavigator nav "//td[. = 'Валюта договора']/following-sibling::td"

        let nmck =
            InlineHtmlNavigator
                nav
                "//td[. = 'Начальная (максимальная) цена договора (предмета закупки):']/following-sibling::td/p/strong"
        ...
        let cusName =
            InlineHtmlNavigator nav "//td[contains(., 'Заказчик:')]/following-sibling::td/strong"
        ...
        let delivTerm =
            InlineHtmlNavigator nav "//td[. = 'Условия и порядок оплаты']/following-sibling::td"
