...
type TenderNeft(stn: Settings.T, tn: NeftRec) =
    ...
    override this.Parsing() =
        ...
        if reader.HasRows then
            ...
            let status =
                ...
                <| doc.QuerySelector("td:contains('Статус закупки') + td span")
            ...
            let descr =
                this.GetDefaultFromNull
                <| doc.QuerySelector("td:contains('Описание') + td span")

            let purObj =
                (sprintf "%s %s" tn.PurName descr).Trim()
            ...
            cmd9.Parameters.AddWithValue("@purchase_object_info", purObj)
            |> ignore
            ...
            if CustomerName <> "" then
                ...
                | false ->
                    ...
                    let inn =
                        this.GetDefaultFromNull
                        <| doc.QuerySelector("td:contains('ИНН заказчика:') + td > p")
            ...
            let lots =
                doc.QuerySelectorAll("span:contains('Лоты') + table tr")
    ...
    member private this.ParsingLots
        (con: MySqlConnection)
        (idTender: int)
        (elem: IElement)
        (lotNum: int)
        ...
        let okpdName =
            this.GetDefaultFromNull
            <| elem.QuerySelector("td:nth-child(2) span")
        ...
        | null ->
            raise
            <| NullReferenceException(sprintf "urlLot not found in %s" urlT)
        | _ -> ()

        let url =
            sprintf "https://zakupki.nefteavtomatika.ru%s" urlT
        ...
        | "" ->
            raise
            <| Exception(sprintf "cannot download page %s" url)
        | _ -> ()
        ...
        let lotName =
            this.GetDefaultFromNull
            <| doc.QuerySelector("td:contains('Наименование лота') + td span")

        let delivPlace =
            this.GetDefaultFromNull
            <| doc.QuerySelector("td:contains('Особые требования') + td span")

        let delivTerm1 =
            this.GetDefaultFromNull
            <| doc.QuerySelector("td:contains('Требуемые сроки поставки') + td span")

        let delivTerm2 =
            this.GetDefaultFromNull
            <| doc.QuerySelector("td:contains('Общие условия оплаты') + td span")

        let delivTerm =
            sprintf "%s %s" delivTerm1 delivTerm2
        ...
        let documents =
            doc.QuerySelectorAll("span:contains('Пакет документов') + table tr")

        for dc in documents do
            let nameF =
                this.GetDefaultFromNull
                <| dc.QuerySelector("td:nth-child(2) span a")

            let desc =
                this.GetDefaultFromNull
                <| dc.QuerySelector("td:nth-child(1) span")
            ...
            if urlAtt <> "" then
                ...
                cmd20.Parameters.AddWithValue("@description", desc)
                |> ignore

                cmd20.ExecuteNonQuery() |> ignore

        let purObjs =
            doc.QuerySelectorAll("span:contains('Список  материалов') + table tr")
        ...
        if purObjs.Length = 0 then
            ...
            for pn in purObjs do
                let purName =
                    this.GetDefaultFromNull
                    <| pn.QuerySelector("td:nth-child(1) span")

                let oq =
                    this.GetDefaultFromNull
                    <| pn.QuerySelector("td:nth-child(2) span")
