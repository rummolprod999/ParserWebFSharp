...
type ParserPik(stn: Settings.T) =
    ...
    member private this.ParserSelen(driver: ChromeDriver) =
        ...
        wait.Until (fun dr ->
            dr
                .FindElement(
                    By.XPath("//app-table-container/app-table-row")
                )
                .Displayed)
        |> ignore
    ...
    member private this.GetNextPage (driver: ChromeDriver) (wait: WebDriverWait) =
        for i in 1..1 do
            try
                ...
                this.Clicker driver
                <| "//button[@class = 'next' and contains(., 'Вперед')]"
    ...
    member private this.ParserTendersList (_: ChromeDriver) (t: PikRec) =
        try
            let T =
                TenderPik(set, t, 125, "ПАО «Группа Компаний ПИК»", "https://tender.pik.ru/")
    ...
    member private this.ParserListTenders(driver: ChromeDriver) =
        let tenders =
            driver.FindElementsByXPath("//app-table-container/app-table-row[position() > 1]")
    ...
    member private this.ParserTenders (driver: ChromeDriver) (i: int) =
        ...
        let t =
            driver.FindElement(By.XPath(sprintf "//app-table-container/app-table-row[position() ='%d']" i))

        let builder = TenderBuilder()

        let result =
            builder {
                let! _ = t.findElementWithoutException ("./app-table-column[1]", sprintf "purNameT not found %s" t.Text)

                let! orgNameT =
                    t.findElementWithoutException ("./app-table-column[3]", sprintf "orgNameT not found %s" t.Text)

                let orgName =
                    orgNameT.Replace("Заказчик", "").Trim()

                let! datePubTT =
                    t.findElementWithoutException ("./app-table-column[4]", sprintf "dates not found %s" t.Text)
                ...
                let! dateEndTT =
                    t.findElementWithoutException ("./app-table-column[5]", sprintf "dates not found %s" t.Text)
                ...
                let pop =
                    driver.FindElement(By.XPath("//app-popup-inner"))

                let personT =
                    pop.findElementWithoutException (".//li[contains(b, 'Контактное лицо:')]")

                let person =
                    personT.Replace("Контактное лицо:", "").Trim()
                ...
                let docs =
                    pop.FindElements(By.XPath(".//li/a"))

                let Docs = List<string>()

                docs
                |> Seq.iter (fun x -> Docs.Add(x.GetAttribute("href")))
                ...
                let close =
                    pop.FindElement(By.XPath("./app-icon[contains(@class, 'close')]"))
