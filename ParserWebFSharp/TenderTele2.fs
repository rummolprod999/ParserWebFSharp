...
type TenderTele2(stn: Settings.T, tn: Tele2Rec, typeFz: int, etpName: string, etpUrl: string) =
    ...
    override this.Parsing() =
        ...
        let res =
                   builder {
                        con.Open()
                        let selectTend = sprintf "SELECT id_tender FROM %stender WHERE purchase_number = @purchase_number AND type_fz = @type_fz" stn.Prefix
    ...
    member private this.GetPos(con: MySqlConnection, idLot: int, idCustomer: int) =
            ...
            if tn.DelivPlace <> "" || tn.DelivTerm <> "" then
                ...
                cmd16.Parameters.AddWithValue("@delivery_term", tn.DelivTerm) |> ignore
