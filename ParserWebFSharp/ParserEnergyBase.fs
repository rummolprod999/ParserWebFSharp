...
type ParserEnergyBase(stn: Settings.T) =
    ...
    let url =
        "https://energybase.ru/tender?page="
    ...
    member private __.ParsingPage(url: string) =
        let Page =
            Download.DownloadUseProxy(Settings.UseProxy, url)
        ...
        | s ->
            ...
            let tens =
                nav
                    .CurrentDocument
                    .DocumentNode
                    .SelectNodesOrEmpty("//div[@class = 'tender-card']")
                    .ToList()
    ...
    member private __.ParsingTender (t: HtmlNode) (_: string) =
        ...
        let res =
            builder {
                let! purName =
                    t.GsnDocWithError ".//div[contains(@class, 'tender-card__title')]"
                    <| sprintf "purName not found %s %s " url (t.InnerText)

                let! hrefT =
                    t.GsnAtrDocWithError ".//div[contains(@class, 'tender-card__title')]/a"
                    <| "href"
                    <| sprintf "hrefT not found %s %s " url (t.InnerText)

                let href =
                    sprintf "https://energybase.ru%s" hrefT

                let! pwName = t.GsnDoc ".//div[contains(@class, 'tender-item__purchase-code-name')]"

                let! purNum =
                    t.GsnDocWithError ".//div[span[contains(., 'Регистрационный номер')]]"
                    <| sprintf "purNum not found %s %s " url (t.InnerText)

                let purNum =
                    purNum.Replace("Регистрационный номер", "").Trim()

                let! datePubT =
                    t.GsnDocWithError ".//span[contains(., 'Дата объявления тендера / закупочной процедуры')]/.."
                ...
                let! datePub =
                    datePubT.DateFromStringDoc(
                        "dd.MM.yyyy г., HH:mm",
                        sprintf "datePub not found %s %s " href (datePubT)
                    )
                ...
                let! dateScoringT =
                    t.GsnDoc ".//span[contains(., 'Дата и время рассмотрения заявок')]/following-sibling::span"

                let dateScoringT =
                    dateScoringT.ReplaceDate()

                let! dateScoring = dateScoringT.DateFromStringDocMin("dd.MM.yyyy г., HH:mm")

                let! cusName =
                    t.GsnDoc ".//span[@class = 'property-label' and contains(., 'Заказчик')]/following-sibling::a"
                ...
                let T =
                    TenderEnergyBase(set, tend, 300, "Energybase.ru", "https://energybase.ru/")
