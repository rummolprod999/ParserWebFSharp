...
type TenderForumGd(stn: Settings.T, tn: ForumGdRec, typeFz: int, etpName: string, etpUrl: string, driver: ChromeDriver) =
    ...
    override this.Parsing() =
        ...
        let res =
            builder {
                ...
                match updated with
                | true -> incr TenderForumGd.tenderUpCount
                | false -> incr TenderForumGd.tenderCount
    ...
    member private this.GetPos(con: MySqlConnection, idLot: int, idCustomer: int) =
        ...
        if tn.DelivPlace <> "" || tn.Period <> "" then
            ...
            cmd16.Parameters.AddWithValue("@delivery_term", tn.Period)
            |> ignore
