...
type ParserTurk(stn: Settings.T) =
    ...
    member private this.ParsingPage(url: string) =
        ...
        | s ->
            ...
            let tens = documents.QuerySelectorAll("div.items > div[data-key]")
            for t in tens do
                try
    ...
    member private this.ParsingTenderAngle (t: IElement) (url: string) =
        ...
        let res = builder {
            let! href = t.GsnAtrDocWithError "div.client-title a" "href" <| sprintf "href not found %s %s " url (t.TextContent)
            let! purNum = href.Get1Doc "(\d+)$" <| sprintf "PurNum not found %s %s " url href
            let! purName = t.GsnDocWithError "div.client-title a" <| sprintf "purName not found %s %s " url (t.TextContent)
