    ...
    override this.Parsing() =
        ...
        let res =
                   builder {
                        ...
                        wait.Until (fun dr -> dr.FindElement(By.XPath("//small[. = ' Информация о тендере']")).Displayed) |> ignore
                        ...
                        let! delivTerm = body.findElementWithoutExceptionOptional("//h4[. = 'Описание тендера:']/following-sibling::div[1]", sprintf <|"delivTerm not found %s" <| tn.Href)
                        if delivTerm <> "" then
                        ...
                        let attachments = body.findElementsWithoutException("//a[starts-with(@href, '/projects-jb/latest-projects-jb/project.download')]")
