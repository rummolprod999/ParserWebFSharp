    ...
    override this.Parsing() =
        ...
        let res =
                   builder {
                        ...
                        let! purName = nav.GsnDocWithError "//h1" <| sprintf "purName not found %s" tn.Href
                        let! datePubT = nav.GsnDocWithError "//div[@class = 'lot_publish_date']/strong" <| sprintf "datePubT not found %s" tn.Href
                        let! datePub = datePubT.DateFromStringDoc("dd.MM.yyyy", sprintf "datePub not found %s %s " tn.Href (datePubT))
                        let! dateEndT1 = nav.GsnDocWithError "//div[@class = 'lot_deadline']/strong" <| sprintf "dateEndT1 not found %s" tn.Href
                        let! dateEndT2 = nav.GsnDocWithError "//div[@class = 'lot_deadline']/span" <| sprintf "dateEndT2 not found %s" tn.Href 
                        let dateEndT = sprintf "%s %s" dateEndT1 dateEndT2
                        let! dateEnd = dateEndT.DateFromStringDoc("dd.MM.yyyy HH:mm", sprintf "dateEnd not found %s %s " tn.Href (dateEndT))
                        let! dateScoringT = nav.GsnDocWithError "//div[@class = 'lot_rez_date']/strong" <| sprintf "dateScoringT not found %s" tn.Href
                        let! dateScoring = dateScoringT.DateFromStringDoc("dd.MM.yyyy", sprintf "dateScoring not found %s %s " tn.Href (datePubT))
                        let status = InlineHtmlNavigator nav "//div[. = 'Статус']/following-sibling::div"
                        con.Open()
                        let selectTend =
                            sprintf 
                        ...
                        let pwName = InlineHtmlNavigator nav "//div[. = 'Вид процедуры']/following-sibling::div"
                        ...
                        let cusName = InlineHtmlNavigator nav "//div[contains(@class, 'row filials')]/span"
                        if cusName <> "" then
                            let selectCustomer =
        ...
        if specUrl <> "" then
            ...
            if lots <> null then
                for l in lots do
                    ...
                    let price = price.Replace("&nbsp;", "").Replace(",", ".").Replace("&le;", "").RegexDeleteWhitespace()
                    let sum = l.Gsn("./td[7]")
                    //let sum = sum.HtmlDecode()
                    let sum = sum.Replace("&nbsp;", "").Replace(",", ".").RegexDeleteWhitespace()
