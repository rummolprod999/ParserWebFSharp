...
type ParserMagnitStroy(stn: Settings.T) =
    ...
    let url =
        "http://tender.magnitostroy.su/projects-jb/latest-projects-jb/project/listproject"
    ...
    member private __.ParserSelen(driver: ChromeDriver) =
        ...
        driver
            .Navigate()
            .GoToUrl("http://tender.magnitostroy.su/")
        ...
        wait.Until (fun dr ->
            dr
                .FindElement(
                    By.XPath("//form/div[@class = 'row-fluid']")
                )
                .Displayed)
        |> ignore
    ...
    member private this.ParserTendersList (driver: ChromeDriver) (t: MagnitStroyRec) =
        try
            let T =
                TenderMagnitStroy(set, t, 309, "ООО \"ТРЕСТ МАГНИТОСТРОЙ\"", "http://tender.magnitostroy.su/", driver)
    ...
    member private this.ParserTenders(i: IWebElement) =
        ...
        let res =
            builder {
                ...
                let! hrefT =
                    i.findWElementWithoutException (".//h3/a", sprintf "hrefT not found, text the element - %s" i.Text)
                ...
                let! nmckT =
                    i.findElementWithoutExceptionOptional (
                        ".//div[.='Начальная цена, Руб']/following-sibling::span",
                ...
                let! contactPerson =
                    i.findElementWithoutExceptionOptional (".//strong[.='Разместил']/following-sibling::a", "")
    ...
    member private __.Auth(driver: ChromeDriver) =
        ...
        driver
            ...
            .SendKeys(Settings.UserMagnit)

        driver
            .FindElement(By.XPath("//input[@id = 'password']"))
            .SendKeys(Settings.PassMagnit)
