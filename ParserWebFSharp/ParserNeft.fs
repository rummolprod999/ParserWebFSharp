...
type ParserNeft(stn: Settings.T) =
    ...
    let url =
        "https://zakupki.nefteavtomatika.ru/zakupki/list?active=1&page="
    ...
    member private this.ParserPage(urlT: string) =
        ...
        | s ->
            ...
            let tens =
                documents.QuerySelectorAll("table.table tr.registerBox")
    ...
    member private this.ParsingTender(t: IElement) =
        ...
        | "" ->
            raise
            <| System.NullReferenceException(sprintf "urlT not found in %s" urlT)
        | _ -> ()
        ...
        let purNumT =
            t.QuerySelector("td:nth-child(1) div b")
        ...
        let purNum =
            purNumT.TextContent.Trim().Replace("№", "")

        let orgNameT =
            t.QuerySelector("td:nth-child(1) div:nth-child(2)")
        ...
        let DateT =
            t.QuerySelector("td:nth-child(3) div")

        match DateT with
        | null ->
            raise
            <| System.NullReferenceException(sprintf "Date not found in %s" url)
        | _ -> ()
        ...
        let pubDateS =
            ...
            | None ->
                raise
                <| System.Exception(sprintf "cannot apply regex to datePub %s" url)
        ...
        let datePub =
            ...
            | None ->
                raise
                <| System.Exception(sprintf "cannot parse datePub %s" pubDateS)
        ...
        let endDateS =
            ...
            | None ->
                raise
                <| System.Exception(sprintf "cannot apply regex to dateEnd %s" url)
        ...
        let dateEnd =
            ...
            | None ->
                raise
                <| System.Exception(sprintf "cannot parse dateEnd %s" endDateS)
