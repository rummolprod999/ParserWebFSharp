...
type TenderNeftReg(stn: Settings.T, tn: NeftRegRec, typeFz: int, etpName: string, etpUrl: string) =
    ...
    override this.Parsing() =
        ...
        let res =
            builder {
                ...
                let orgName =
                    InlineHtmlNavigator nav "//td[contains(., 'Наименование организации')]/following-sibling::td"
                ...
                if orgName <> "" then
                    ...
                    | false ->
                        ...
                        let postAddress =
                            InlineHtmlNavigator nav "//td[contains(., 'Почтовый адрес')]/following-sibling::td"

                        let factAddress =
                            InlineHtmlNavigator nav "//td[contains(., 'Место нахождения')]/following-sibling::td"

                        let phone =
                            InlineHtmlNavigator nav "//td[contains(., 'Телефон / Факс')]/following-sibling::td"
                ...
                let! datePubT =
                    nav.GsnDocWithError "//h4[. = 'Дата начала / окончания подачи заявок']/following-sibling::span[1]"
                    <| sprintf "datePubT not found %s" tn.Href

                let! datePub =
                    datePubT.DateFromStringDoc(
                        "dd.MM.yyyy / HH:mm",
                        sprintf "datePub not found %s %s " tn.Href datePubT
                    )

                let! dateEndT =
                    nav.GsnDocWithError "//h4[. = 'Дата начала / окончания подачи заявок']/following-sibling::span[2]"
                ...
                match updated with
                | true -> incr TenderNeftReg.tenderUpCount
                | false -> incr TenderNeftReg.tenderCount

                this.GetAttachments(con, !idTender, htmlDoc)
                this.GetLots(con, !idTender, htmlDoc, orgName)
    ...
    member private this.GetAttachments(con: MySqlConnection, idTender: int, doc: HtmlDocument) =
        ...
        let docs =
            doc.DocumentNode.SelectNodes("//div[@class = 'fileloads']/a")
                    ...
                    if docName <> "" && docUrl <> "" then
                        let d =
                            { name = docName.RegexCutWhitespace()
                              url = sprintf "https://etp.neftregion.ru%s" docUrl }
    ...
    member private this.GetLots(con: MySqlConnection, idTender: int, doc: HtmlDocument, orgName: string) =
        ...
        cmd12.Parameters.AddWithValue("@finance_source", "")
        |> ignore
        ...
        cmd19.Parameters.AddWithValue("@sum", tn.Nmck)
        |> ignore
