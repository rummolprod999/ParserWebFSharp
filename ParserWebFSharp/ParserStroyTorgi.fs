...
type ParserStroyTorgi(stn: Settings.T) =
    ...
    let CreateDriver opt =
        lazy (new ChromeDriver("/usr/local/bin", opt))
    ...
    member private this.ParserList() =
        ...
        | s ->
            ...
            let tens =
                documents.QuerySelectorAll("ul.table-auction li[data-href^= '/trades/']")
    ...
    member private this.ParsingTender(t: IElement) =
        let UrlT =
            t.GetAttribute("data-href").Trim()

        match UrlT with
        | ""
        | null ->
            raise
            <| NullReferenceException(sprintf "UrlT not found in %s" url)
        | _ -> ()

        let Url =
            sprintf "https://stroytorgi.ru%s" UrlT
        ...
        let purNameT =
            t.QuerySelectorAll("div:nth-child(2) div:nth-child(2) ul li a")
        ...
        |> Seq.iter (fun i ->
            purName
            := sprintf "%s %s" !purName (i.TextContent.Trim()))
        ...
        try
            let T =
                TenderStroyTorgi(set, ten, 69, "ЭТП «СтройТорги»", "https://stroytorgi.ru/trades")
