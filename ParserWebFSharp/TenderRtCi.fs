...
type TenderRtCi(stn: Settings.T, tn: RtCiRec, typeFz: int, etpName: string, etpUrl: string) =
    ...
    override this.Parsing() =
        ...
        let res =
                   builder {
                        ...
                        let orgName = InlineHtmlNavigator nav "(//div[@class = 'fields-table__label text-default' and contains(., 'Наименование организации')]/following-sibling::div)[1]"
                        ...
                        if orgName <> "" then
                            ...
                            | false ->
                                ...
                                let contactPerson = InlineHtmlNavigator nav "(//div[@class = 'fields-table__label text-default' and contains(., 'Контактное лицо')]/following-sibling::div)[2]"
                                let postAddress = InlineHtmlNavigator nav "(//div[@class = 'fields-table__label text-default' and contains(., 'Почтовый адрес')]/following-sibling::div)[2]"
                                let factAddress = InlineHtmlNavigator nav "(//div[@class = 'fields-table__label text-default' and contains(., 'Место нахождения')]/following-sibling::div)[2]"
                                let phone = InlineHtmlNavigator nav "(//div[@class = 'fields-table__label text-default' and contains(., 'Телефон')]/following-sibling::div)[2]"
                                let inn = ""
                        ...
                        let scoringDateT = InlineHtmlNavigator nav "//div[@class = 'fields-table__label text-default' and contains(., 'Дата рассмотрения, оценки и')]/following-sibling::div"
                        let scoringDate = scoringDateT.DateFromStringOrMin("dd.MM.yyyy")
                        let idTender = ref 0
                        let numVersion = 1
                        let status = ""
                        ...
                        let cusName = InlineHtmlNavigator nav "(//div[@class = 'fields-table__label text-default' and contains(., 'Наименование организации')]/following-sibling::div)[1]"
    ...
    member private this.GetAttachments(con: MySqlConnection, idTender: int, doc: HtmlDocument) =
        let documents = doc.DocumentNode.SelectNodes("//a[@class = 'link-action' and contains(@href, 'upload')]")
        if documents <> null then
            for d in documents do
                let docName = d.Gsn(".//span")
                let docUrl = d.GsnAtr "." "href"
                let docUrl = sprintf "https://zakupki.rt-ci.ru%s" docUrl
                if docUrl <> "" then
    ...
    member private this.GetLots(con: MySqlConnection, idTender: int, idCustomer, doc: HtmlDocument) =
        let lotNum = ref 1
        let lots = doc.DocumentNode.SelectNodes("//div[contains(@class, 'notification-lots__item')]")
        if lots <> null then
            for l in lots do
                let idLot = ref 0
                let lotName = l.Gsn(".//div[contains(@class, 'accordion-notification-lots__desc')]")
                let lotName = lotName.HtmlDecode()
                let NmckT = l.Gsn(".//div[@class = 'fields-table__label text-default' and contains(., 'Начальная стоимость')]/following-sibling::div")
                let nmckList = NmckT.Get2ListRegexp "([\d,\s]+)\s+\((.+)\)"
                ...
                cmd12.Parameters.AddWithValue("@currency", nmckList.[1]) |> ignore
                cmd12.Parameters.AddWithValue("@lot_name", lotName) |> ignore
                cmd12.ExecuteNonQuery() |> ignore
                idLot := int cmd12.LastInsertedId
                incr lotNum
                let OkeiT = l.Gsn(".//div[@class = 'fields-table__label text-default' and contains(., 'Количество товара и код ОКЕИ')]/following-sibling::div")
                let okeiList = OkeiT.Get2ListRegexp "([\d,\s]+)\s+(.+)"
                let quantity = okeiList.[0].GetPriceFromString()
                ...
                cmd19.Parameters.AddWithValue("@okei", okeiList.[1]) |> ignore
                cmd19.Parameters.AddWithValue("@okpd2_code", "") |> ignore
                cmd19.ExecuteNonQuery() |> ignore
                let delivPlace = l.Gsn(".//div[@class = 'fields-table__label text-default' and contains(., 'Место поставки товара')]/following-sibling::div")
                if delivPlace <> "" then
