...
type TenderForScience(stn: Settings.T, tn: ForScienceRec, typeFz: int, etpName: string, etpUrl: string) =
    ...
    override this.Parsing() =
        ...
        let res =
            builder {
                ...
                match updated with
                | true -> incr TenderForScience.tenderUpCount
                | false -> incr TenderForScience.tenderCount
    ...
    member private this.GetAttachments(con: MySqlConnection, idTender: int, doc: HtmlDocument) =
        ...
        let docs =
            doc.DocumentNode.SelectNodes("//div[@class = 'files-download-block-in']/a")
                    ...
                    if docName <> "" && docUrl <> "" then
                        let d =
                            { name = docName.RegexCutWhitespace()
                              url = sprintf "https://4science.ru%s" docUrl }
