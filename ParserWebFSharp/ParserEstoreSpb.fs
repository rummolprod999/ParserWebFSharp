...
type ParserEstoreSpb(stn: Settings.T) =
    ...
    member private __.ParsingTender (t: IElement) (url: string) =
        ...
        let res = builder {
            ...
            let! orgName = t.GsnDocWithError "td:nth-of-type(3)" <| sprintf "orgName not found %s %s " url (t.TextContent)
            let! dateEndT = t.GsnDocWithError "td:nth-of-type(5)" <| sprintf "dateEndT not found %s %s " url (t.TextContent)
            let! dateEnd = dateEndT.DateFromStringDoc ("HH:mm dd.MM.yyyy", sprintf "dateEnd not found %s %s " href dateEndT)
            let! datePubT = t.GsnDocWithError "td:nth-of-type(4)" <| sprintf "datePubT not found %s %s " url (t.TextContent)
            let! datePub = datePubT.DateFromStringDoc ("HH:mm dd.MM.yyyy", sprintf "datePub not found %s %s " href datePubT)
            let! nmck = t.GsnDocWithError "td:nth-of-type(6)" <| sprintf "nmck not found %s %s " url (t.TextContent)
            let nmck = nmck.GetPriceFromStringKz()
            let! status = t.GsnDocWithError "td:nth-of-type(7)" <| sprintf "status not found %s %s " url (t.TextContent)
            let tend = {Href = href
                        DateEnd = dateEnd
                        DatePub = datePub
                        PurName = purName
