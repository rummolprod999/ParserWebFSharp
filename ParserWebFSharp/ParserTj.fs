...
type ParserTj(stn: Settings.T) =
    ...
    let spage =
        "http://test.zakupki.gov.tj/reestr-zakazov-v-elektronnoy-forme/"
    ...
    member __.GetNextpage(driver: ChromeDriver) =
        ...
        try
            if two then
                jse.ExecuteScript("document.querySelector('div.links button:nth-of-type(2)').click()", "")
                |> ignore

                two <- false
            else
                jse.ExecuteScript("document.querySelector('div.links button:nth-of-type(4)').click()", "")
                |> ignore
    ...
    member private __.ParserListTenders(driver: ChromeDriver) =
        __.Wait.Until (fun dr ->
            dr
                .FindElement(
                    By.XPath("//table[@class = 'aqua_table']/tbody/tr[not(@valign)][10]")
                )
                .Displayed)
        |> ignore
    ...
    member private __.GetContentTender (driver: ChromeDriver) (i: int) =
        ...
        while wh do
            try
                ...
                let t =
                    driver.FindElement(By.XPath(sprintf "//table[@class = 'aqua_table']/tbody/tr[not(@valign)][%d]" i))
            ...
            builder {
                let! purNumT =
                    i.findElementWithoutException (
                        ".//td[2]/strong",
                        sprintf "purNumT not found, inner text - %s" i.Text
                    )
                ...
                let! purName = i.findElementWithoutException (".//td[2]/a[1]", sprintf "purName not found %s" i.Text)

                let! orgName =
                    i.findElementWithoutException (".//td[2]/strong[2]", sprintf "orgName not found %s" i.Text)

                let! status = i.findElementWithoutException (".//td[1]", sprintf "status not found %s" i.Text)
