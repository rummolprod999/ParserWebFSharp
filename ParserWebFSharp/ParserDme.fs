...
type ParserDme(stn: Settings.T) =
    ...
    let url =
        "https://market.dme.aero/purchase.aspx"
        ...
        let res =
            builder {
                ...
                let! purType =
                    ...
                    <| sprintf "purType not found %s %s " url (t.InnerText)
                ...
                let! cusName =
                    t.GsnDocWithError "./td[4]/a"
                    <| sprintf "cusName not found %s %s " url (t.InnerText)

                let! dateEndT =
                    t.GsnDocWithError "./td[5]"
                ...
                let! personName =
                    t.GsnDocWithError "./td[6]"
                    <| sprintf "personName not found %s %s " url (t.InnerText)

                let! personEmail = (t.GsnDoc "./td[7]/div[contains(., 'Почта:')]")

                let personEmail =
                    personEmail.Replace("Почта:", "").Trim()

                let! personPhone = (t.GsnDoc "./td[7]/div[contains(., 'телефон:')]")

                let personPhone =
                    personPhone
                        .Replace("Рабочий телефон:", "")
                        .Replace("Мобильный телефон:", "")
                        .Trim()
                ...
                let T =
                    TenderDme(set, tend, 285, "Московский аэропорт Домодедово", "https://market.dme.aero/")
