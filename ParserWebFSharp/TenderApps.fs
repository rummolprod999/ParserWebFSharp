...
type TenderApps(stn: Settings.T, tn: AppsRec, typeFz: int, etpName: string, etpUrl: string) =
        ...
        let res =
            builder {
                ...
                let datePubT =
                    InlineHtmlNavigator
                        nav
                        "//div[contains(., 'Дата тендера') and @class = 'editor-label']/following-sibling::div[1]"
                ...
                let dateEndT =
                    InlineHtmlNavigator
                        nav
                        "//div[contains(., 'Дата окончания приема предложений') and @class = 'editor-label']/following-sibling::div[1]"

                let! dateEnd =
                    dateEndT.DateFromStringDoc("dd.MM.yyyy", sprintf "dateEnd not found %s %s " tn.Href dateEndT)

                let status =
                    InlineHtmlNavigator
                        nav
                        "//div[contains(., 'Стадия тендера') and @class = 'editor-label']/following-sibling::div[1]"
                ...
                let pwName =
                    InlineHtmlNavigator
                        nav
                        "//div[contains(., 'Тип конкурса') and @class = 'editor-label']/following-sibling::div[1]"
    ...
    member private this.GetLots(con: MySqlConnection, idTender: int, doc: HtmlDocument) =
        ...
        let delivTerm1 =
            InlineHtmlNavigator
                nav
                "//div[contains(., 'Дата начала поставки') and @class = 'editor-label']/following-sibling::div[1]"

        let delivTerm2 =
            InlineHtmlNavigator
                nav
                "//div[contains(., 'Дата окончания поставки') and @class = 'editor-label']/following-sibling::div[1]"

        let delivTerm =
            sprintf "Дата начала поставки %s, Дата окончания поставки %s" delivTerm1 delivTerm2
        ...
        let requirement =
            InlineHtmlNavigator
                nav
                "//div[contains(., 'Требование к поставщику') and @class = 'editor-label']/following-sibling::div[1]"
        ...
        if requirement <> "" then
            ...
            cmd.Parameters.AddWithValue("@content", requirement)
            |> ignore
        ...
        let purObjs =
            doc.DocumentNode.SelectNodes(
                "//legend[contains(., 'Строки')]//following-sibling::table//tr[position() > 1]"
            )
