...
type TenderRtsCorp(stn: Settings.T, tn: RtsCorpRec, typeFz: int, etpName: string, etpUrl: string) =
    ...
    override this.Parsing() =
        ...
        let res =
            builder {
                ...
                let pubDateT =
                    nav.Gsn ".//td[contains(., 'Дата публикации')]/following-sibling::td"
                ...
                let endDateT =
                    nav.Gsn ".//td[contains(., 'Дата окончания подачи заявок')]/following-sibling::td"
                ...
                let orgName =
                    nav.Gsn "//td[contains(., 'Организатор')]/following-sibling::span/a/span"
                ...
                let pwName =
                    InlineHtmlNavigator nav "//td[contains(., 'Правило проведения')]/following-sibling::td/span/a/span"
    ...
    member private this.GetAttachments(con: MySqlConnection, idTender: int, _: string, doc: HtmlDocument) =
        let documents =
            doc.DocumentNode.SelectNodes(
                "//div[@class = 'header2' and . = 'Документы']/following-sibling::div//table/tbody/tr"
            )
    ...
    member private this.Lot(con: MySqlConnection, idTender: int, l: HtmlNode, lotNum: int) =
        ...
        let cusName =
            l.Gsn "//td[contains(., 'Заказчик')]/following-sibling::td"
        ...
        let purObjects =
            l.SelectNodes(
                ".//div[@class = 'header2' and contains(., 'Информация об объектах')]/following-sibling::div//table/tbody/tr"
            )
        ...
        if purObjects <> null then
            for po in purObjects do
                ...
                let quantity =
                    po.Gsn("./td[4]").GetPriceFromString()
