...
type TenderSibGenco(stn: Settings.T, tn: SibGencoRec, typeFz: int, etpName: string, etpUrl: string) =
    ...
    override this.Parsing() =
        ...
        let res =
                   builder {
                        ...
                        let orgName = InlineHtmlNavigator nav "//div[@class = 'info-table__label h4' and .  = 'Организатор:']/following-sibling::div"
                        let IdOrg = ref 0
                        if orgName <> "" then
                            ...
                            | false ->
                                ...
                                let contactPerson = InlineHtmlNavigator nav "//div[@class = 'info-table__label h4' and .  = 'Контактное лицо:']/following-sibling::div"
                                let postAddress = ""
                                let factAddress = ""
                                let phone = InlineHtmlNavigator nav "//div[@class = 'info-table__label h4' and .  = 'Контактный телефон:']/following-sibling::div"
                                let inn = ""
                                let email = InlineHtmlNavigator nav "//div[@class = 'info-table__label h4' and .  = 'E-mail:']/following-sibling::div"
                        ...
                        let scoringDateT = InlineHtmlNavigator nav "//td[. = 'Дата подведения итогов']/following-sibling::td"
                        ...
                        let cusName = InlineHtmlNavigator nav "//td[. = 'Наименование организации']/following-sibling::td"
    ...
    member private this.GetAttachments(con: MySqlConnection, idTender: int, doc: HtmlDocument) =
        let documents = doc.DocumentNode.SelectNodes("//a[@class = 'doc link']")
        if documents <> null then
            let documents = documents.Skip(1)
            for d in documents do
                let docName = d.Gsn(".//div[@class = 'doc__title']")
                let docUrl = d.GsnAtr "." "href"
                let docUrl = sprintf "https://sibgenco.ru%s" docUrl
                if docUrl <> "" then
