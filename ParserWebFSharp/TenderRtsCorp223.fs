...
type TenderRtsCorp223(stn: Settings.T, tn: RtsCorpRec, typeFz: int, etpName: string, etpUrl: string) =
    ...
    override this.Parsing() =
        ...
        let res =
            builder {
                ...
                let pubDateT =
                    nav.Gsn ".//label[contains(., 'Дата и время начала подачи заявок')]/following-sibling::span"
                ...
                let endDateT =
                    nav.Gsn
                        ".//label[contains(., 'Дата и время окончания срока подачи заявок')]/following-sibling::span"
                ...
                let orgName =
                    nav.Gsn "//label[contains(., 'Наименование организации')]/following-sibling::span/a"
                ...
                if orgName <> "" then
                    ...
                    | false ->
                        ...
                        let contactPerson =
                            InlineHtmlNavigator nav "//label[contains(., 'ФИО')]/following-sibling::span"

                        let postAddress =
                            InlineHtmlNavigator nav "//label[contains(., 'Почтовый адрес')]/following-sibling::span"

                        let factAddress =
                            InlineHtmlNavigator nav "//label[contains(., 'Место нахождения')]/following-sibling::span"

                        let phone =
                            InlineHtmlNavigator nav "//label[contains(., 'Телефон')]/following-sibling::span"

                        let inn =
                            InlineHtmlNavigator nav "//label[contains(., 'ИНН организации')]/following-sibling::span"

                        let email =
                            InlineHtmlNavigator nav "//label[contains(., 'E-mail адрес')]/following-sibling::span"
                ...
                let pwName =
                    InlineHtmlNavigator nav ".//label[contains(., 'Тип закупки')]/following-sibling::span"
                ...
                match updated with
                | true -> incr TenderRtsCorp223.tenderUpCount
                | false -> incr TenderRtsCorp223.tenderCount
    ...
    member private this.GetLots(con: MySqlConnection, idTender: int, doc: HtmlDocument) =
        ...
        let lots =
            doc.DocumentNode.SelectNodes("//div[contains(@class, 'tradeLotInfo')]")
    ...
    member private this.Lot(con: MySqlConnection, idTender: int, l: HtmlNode, lotNum: int) =
        ...
        let nmckT =
            l.Gsn ".//label[contains(., 'Начальная (минимальная) цена')]/following-sibling::span"
        ...
        let lotName =
            l.Gsn ".//label[contains(., 'Наименование')]/following-sibling::span"
        ...
        if cusName <> "" then
            ...
            | false ->
                ...
                let inn =
                    l.Gsn ".//label[contains(., 'ИНН')]/following-sibling::span"
        ...
        let delivPlace =
            l.Gsn ".//label[contains(., 'Место поставки')]/following-sibling::span"

        let delivTerm =
            l.Gsn ".//label[contains(., 'Сроки поставки')]/following-sibling::span"

        let applAmount =
            l.Gsn ".//label[contains(., 'Обеспечение заявки')]/following-sibling::span"

        let applAmount =
            applAmount.GetPriceFromString()

        let contrAmount =
            l.Gsn ".//label[contains(., 'Обеспечение договора')]/following-sibling::span"
        ...
        let purObjects =
            l.SelectNodes(".//table[contains(., 'ОКПД2')]//tr[position() > 0]")
