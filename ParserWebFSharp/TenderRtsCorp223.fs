...
type TenderRtsCorp223(stn: Settings.T, tn: RtsCorpRec, typeFz: int, etpName: string, etpUrl: string) =
    ...
    override this.Parsing() =
        ...
        let res =
                   builder {
                        ...
                        let pubDateT = nav.Gsn ".//label[contains(., 'Дата и время начала подачи заявок')]/following-sibling::span"
                        ...
                        let endDateT = nav.Gsn ".//label[contains(., 'Дата и время окончания срока подачи заявок')]/following-sibling::span"
                        ...
                        let pwName = InlineHtmlNavigator nav".//label[contains(., 'Тип закупки')]/following-sibling::span"
                        match pwName with
                        | "" -> ()
    ...
    member private this.GetLots(con: MySqlConnection, idTender: int, doc: HtmlDocument) =
        ...
        let lotNum = ref 1
        let lots = doc.DocumentNode.SelectNodes("//div[contains(@class, 'tradeLotInfo')]")
        for l in lots do
            try
    ...
    member private this.Lot(con: MySqlConnection, idTender: int, l: HtmlNode, lotNum: int) =
        ...
        let cusName = l.Gsn ".//label[contains(., 'Заказчик')]/following-sibling::span/a"
        ...
        let delivPlace = l.Gsn ".//label[contains(., 'Место поставки')]/following-sibling::span"
        let delivTerm = l.Gsn ".//label[contains(., 'Сроки поставки')]/following-sibling::span"
        ...
        let purObjects = l.SelectNodes (".//table[contains(., 'ОКПД2')]//tr[position() > 0]")
