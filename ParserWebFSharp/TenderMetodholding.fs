...
type TenderMetodholding(stn: Settings.T, tn: MetodholdingRec, typeFz: int, etpName: string, etpUrl: string) =
    ...
    override this.Parsing() =
        ...
        let res =
                   builder {
                        ...
                        match updated with
                        | true -> incr TenderMetodholding.tenderUpCount
                        | false -> incr TenderMetodholding.tenderCount
                        let idLot = ref 0
                        let currency = ""
                        let finSource = ""
                        ...
                        let purObjs = document.QuerySelectorAll("table.info_table tbody tr").ToList().Skip(1)
                        this.PurObjs (con, !idLot, !idCustomer, purObjs)
    ...
    member private this.PurObjs(con: MySqlConnection, idLot: int, idCus, j) =
        for p in j do
            let poName = p.QuerySelector("td:nth-child(3)").TextContent.Trim()
            let quant = p.QuerySelector("td:nth-child(5)").TextContent.Trim()
            let okei = p.QuerySelector("td:nth-child(4)").TextContent.Trim()
            let kat = p.QuerySelector("td:nth-child(2)").TextContent.Trim()
            let poName = (sprintf "%s: %s" kat poName).Trim()
            ...
            let delivTerm = p.QuerySelector("td:nth-child(6)").TextContent.Trim()
            if delivTerm <> "" then
                let insertCustomerRequirement = sprintf "INSERT INTO %scustomer_requirement SET id_lot = @id_lot, id_customer = @id_customer, delivery_place = @delivery_place, application_guarantee_amount = @application_guarantee_amount, contract_guarantee_amount = @contract_guarantee_amount, delivery_term = @delivery_term" stn.Prefix
                let cmd16 = new MySqlCommand(insertCustomerRequirement, con)
                cmd16.Prepare()
                cmd16.Parameters.AddWithValue("@id_lot", idLot) |> ignore
                cmd16.Parameters.AddWithValue("@id_customer", idCus) |> ignore
