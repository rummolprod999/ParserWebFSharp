...
type TenderMetodholding(stn: Settings.T, tn: MetodholdingRec, typeFz: int, etpName: string, etpUrl: string) =
    ...
    override this.Parsing() =
        ...
        let res =
            builder {
                ...
                let Page =
                    Download.DownloadPost(parametrs, tn.Href)
                ...
                let purObjs =
                    document
                        .QuerySelectorAll("table.info_table tbody tr")
                        .ToList()
                        .Skip(1)
    ...
    member private this.PurObjs(con: MySqlConnection, idLot: int, idCus, j) =
        for p in j do
            ...
            cmd19.Parameters.AddWithValue("@id_customer", idCus)
            |> ignore
            ...
            if delivTerm <> "" then
                ...
                cmd16.Parameters.AddWithValue("@id_customer", idCus)
                |> ignore
