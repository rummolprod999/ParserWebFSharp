...
type ParserSlav(stn: Settings.T) =
    ...
    override this.Parsing() =
        try
            let url =
                "http://sn-mng.ru/zakupki-i-realizatsiya/zakupki/"
        ...
        try
            let url =
                "https://www.slavneft.ru/supplier/procurement/"
    ...
    member private this.ParserMegion(url: string) =
        ...
        | s ->
            ...
            let tens =
                documents.QuerySelectorAll("div.zakupka_single_wrap")
    ...
    member private this.ParsingTenderMegion (t: IElement) (url: string) =
        ...
        let HrefDoc =
            sprintf "http://sn-mng.ru%s" HrefDocT

        let NameDoc =
            match t.QuerySelector("div.tab_docs div.file_wrap a") with
            | null ->
                raise
                <| NullReferenceException(sprintf "NameDoc not found in %s" url)
        ...
        let purNum =
            PurNumT.Replace("ПДО №", "").Trim()

        let CusName =
            match t.QuerySelector("div:contains('Заказчик') + div") with
            | null ->
                raise
                <| NullReferenceException(sprintf "CusName not found in %s" url)
            | ur -> ur.TextContent.Trim()
        ...
        try
            let T =
                TenderSlav(set, ten, 56, "ОАО «Славнефть-Мегионнефтегаз»", url)
    ...
    member private this.ParserYanos(url: string) =
        ...
        | s ->
            ...
            let tens =
                documents.QuerySelectorAll("div.tender__inside")
    ...
    member private this.ParsingTenderYanos (t: IElement) (url: string) =
        ...
        | null ->
            raise
            <| NullReferenceException(sprintf "HrefDocT not found in %s %s" url t.TextContent)
        | _ -> ()

        let HrefDoc =
            sprintf "http://www.refinery.yaroslavl.su%s" HrefDocT
        ...
        try
            let T =
                TenderSlav(set, ten, 57, "ОАО «Славнефть-ЯНОС»", url)
    ...
    member private this.ParserNgre(url: string) =
        ...
        | s ->
            ...
            let tens =
                documents.QuerySelectorAll("table.contest tr:not(th)")
    ...
    member private this.ParsingTenderNgre (t: IElement) (url: string) =
        ...
        let HrefDoc =
            sprintf "https://www.slavneft.ru%s" HrefDocT
        ...
        let OrgName =
            ...
            | null ->
                raise
                <| NullReferenceException(sprintf "OrgName not found in %s" url)
            | ur -> ur.TextContent.Trim()
        ...
        try
            let T =
                TenderSlav(set, ten, 58, " ООО «Байкитская НГРЭ»", url)
