...
type TenderButb
    ...
    override this.Parsing() =
        ...
        if reader.HasRows then
            ...
            wait.Until (fun dr ->
                dr
                    .FindElement(
                        By.XPath("//input[@value = 'Вернуться']")
                    )
                    .Displayed)
            |> ignore
            ...
            let paginator =
                this.GetDefaultFromNullS
                <| this.checkElement (driver, "//span[@class = 'paginator']")
    ...
    member private this.ParsingDocs (con: MySqlConnection) (idTender: int) (elem: IWebElement) =
        let docName =
            this.GetDefaultFromNullS
            <| this.checkElement (elem, ".//td[3]")

        match docName with
        | "" -> ()
        | x ->
            let hrefTT =
                this.checkElement (elem, ".//td[4]//a")
            ...
            cmd5.Parameters.AddWithValue("@description", descr)
            |> ignore
    ...
    member private this.GetLots
        (_: ChromeDriver, idTender: int, idCustomer: int, requirement: string, con: MySqlConnection, pricecheck: bool)
        (elem: IWebElement)
        =
        let idLot = ref 0
        ...
        let lotNumT =
            this.GetDefaultFromNullS
            <| this.checkElement (elem, ".//td[1]/span")
        ...
        if pricecheck then
            let priceTT =
                this.GetDefaultFromNullS
                <| this.checkElement (elem, ".//td[8]/span[1]")

            price <- this.GetPrice(priceTT)

        let finsource =
            this.GetDefaultFromNullS
            <| this.checkElement (elem, ".//td[7]/span")

        let currency =
            this.GetDefaultFromNullS
            <| this.checkElement (elem, ".//td[8]/span[2]")
        ...
        let okpd2 =
            this.GetDefaultFromNullS
            <| this.checkElement (elem, ".//td[2]/span")

        let name =
            this.GetDefaultFromNullS
            <| this.checkElement (elem, ".//td[3]/a/span")

        let quantityT =
            this.GetDefaultFromNullS
            <| this.checkElement (elem, ".//td[4]/span")
        ...
        let delivPlace =
            this.GetDefaultFromNullS
            <| this.checkElement (elem, ".//td[5]/span")

        let delivTerm =
            this.GetDefaultFromNullS
            <| this.checkElement (elem, ".//td[6]/span")
