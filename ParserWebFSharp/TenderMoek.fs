...
type TenderMoek(stn: Settings.T, tn: MoekRec, typeFz: int, etpName: string, etpUrl: string) =
    ...
    override this.Parsing() =
        ...
        let res =
                   builder {
                        ...
                        let pwName = InlineHtmlNavigator nav "//span[contains(., 'Способ закупок')]/following-sibling::span"
                        match pwName with
                        | "" -> ()
                        ...
                        match updated with
                        | true -> incr TenderMoek.tenderUpCount
                        | false -> incr TenderMoek.tenderCount
    ...
    member private this.GetAttachments(con: MySqlConnection, idTender: int, doc: HtmlDocument) =
            ...
            let docs = doc.DocumentNode.SelectNodes("//span[@class = 'file zip']/a")
            match docs with
            | null -> ()
            | _ -> 
                for doc in docs do
                        ...
                        if docName <> "" && docUrl <> "" then 
                            let d =
                                { name = docName.RegexCutWhitespace()
                                  url = sprintf "https://www.moek.ru%s" docUrl }
                            docList.Add(d)
