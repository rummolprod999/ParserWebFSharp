...
type ParserSngb(stn: Settings.T) =
    ...
    member private __.ParsingPage(url: string) =
        ...
        | s ->
            ...
            let tens = nav.CurrentDocument.DocumentNode.SelectNodesOrEmpty("//table[@class = 'table_fix']/tbody").ToList()
            tens.Reverse()
            for t in tens do
                    try
    ...
    member private __.ParsingTender (t: HtmlNode) (url: string) =
        ...
        let res = builder {
            let! purName = t.GsnDocWithError ".//td[contains(., 'Предмет тендера')]/following-sibling::td" <| sprintf "purName not found %s %s " url (t.InnerText)
            ...
            let dateEnd = match dateEndT.DateFromStringOrMin("dd.MM.yyyy") with
            ...
            | x when (not <| String.IsNullOrEmpty(x)) -> delivTerm.Append("Порядок оплаты: ").Append(x).AppendLine() |> ignore
            | _ -> ()
            match t.Gsn ".//td[contains(., 'Срок исполнения обязательства')]/following-sibling::td" with
            | x when (not <| String.IsNullOrEmpty(x)) -> delivTerm.Append("Срок исполнения обязательства: ").Append(x).AppendLine() |> ignore
            | _ -> ()
            match t.Gsn ".//td[contains(., 'Гарантийный срок')]/following-sibling::td" with
            | x when (not <| String.IsNullOrEmpty(x)) -> delivTerm.Append("Гарантийный срок: ").Append(x).AppendLine() |> ignore
            | _ -> ()
            match t.Gsn ".//td[contains(., 'Дополнительные требования')]/following-sibling::td" with
            | x when (not <| String.IsNullOrEmpty(x)) -> delivTerm.Append("Дополнительные требования: ").Append(x).AppendLine() |> ignore
            | _ -> ()
            match t.Gsn ".//td[contains(., 'Дополнительная информация')]/following-sibling::td" with
            | x when (not <| String.IsNullOrEmpty(x)) -> delivTerm.Append("Дополнительная информация: ").Append(x).AppendLine() |> ignore
            | _ -> ()
            match t.Gsn ".//td[contains(., 'Порядок оплаты')]/following-sibling::td" with
            | x when (not <| String.IsNullOrEmpty(x)) -> delivTerm.Append("Порядок оплаты: ").Append(x).AppendLine() |> ignore
            | _ -> ()
            match t.Gsn ".//td[contains(., 'Прочее')]/following-sibling::td" with
            | x when (not <| String.IsNullOrEmpty(x)) -> delivTerm.Append("Прочее: ").Append(x).AppendLine() |> ignore
            | _ -> ()
            ...
            let tend ={   Href = href
                          ...
                          DatePub = datePub
                          DateEnd = dateEnd}
            let T = TenderSngb(set, tend, 359, "АО БАНК «СНГБ»", "https://www.sngb.ru/")
            T.Parsing()
            return ""
        }
        match res with
        ...
        | _ -> 
            for doc in docs do
                try 
                    ...
                    let mutable docUrl = doc.getAttrWithoutException ("href")
                    if not <| docUrl.StartsWith("https://www.sngb.ru") then docUrl <- sprintf "%s%s" "https://www.sngb.ru" docUrl
