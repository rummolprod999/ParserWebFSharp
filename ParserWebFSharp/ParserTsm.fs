...
type ParserTsm(stn: Settings.T) =
    ...
    let url =
        "https://tender.tsm.ru/trades?page=purchases"
    ...
    member private this.ParserSelen(driver: ChromeDriver) =
        ...
        wait.Until (fun dr ->
            dr
                .FindElement(
                    By.XPath("//div[@id = 'trades']//um-trade-list-item")
                )
                .Displayed)
        |> ignore
    ...
    member private this.ParserTendersList (driver: ChromeDriver) (t: SamoletRec) =
        try
            let T =
                TenderTsm(set, t, 185, "ООО \"Трансстроймеханизация\"", "https://tender.tsm.ru/", driver)
    ...
    member private this.ParserTenders (_: ChromeDriver) (i: IWebElement) =
        ...
        let result =
            builder {
                let! hrefT = i.findWElementWithoutException (".//a", sprintf "hrefT not found %s" i.Text)
                ...
                let! purNum =
                    i.findElementWithoutException (
                        ".//div[@class = 'header-row']/span[contains(@class, 'registered-number')]",
