...
type TenderAero(stn : Settings.T, tn : AeroRec, typeFz : int, etpName : string, etpUrl : string) =
    ...
    member private this.ParsingDocs (con : MySqlConnection) (idTender : int) (elem : IElement) =
        ...
        | "" -> ()
        | x -> 
            let hrefT = elem.QuerySelector("td a").GetAttribute("href")
            let href = sprintf "https://www.aeroflot.ru%s" hrefT
            let addAttach =
    ...
    override this.Parsing() =
        ...
        if reader.HasRows then reader.Close()
            ...
            | false -> 
                ...
                let addOrganizer =
                    sprintf 
                        ...
                        stn.Prefix
                let contactPerson = ""
                let postAddress = "Российская Федерация, 119002, г. Москва, ул. Арбат, д. 10"
                let factAddress = "Российская Федерация, 119002, г. Москва, ул. Арбат, д. 10"
                let phone = ""
                let inn = "7712040126"
            ...
            let CustomerName =
                match doc.QuerySelector
                          ("span:contains('Дополнительная информация') + table td:contains('Заказчик:') + td") with
                | null -> ""
                | ur -> ur.TextContent.Trim()
