...
type TenderYarRegion(stn : Settings.T, tn : YarRegionRec, typeFz : int, etpName : string, etpUrl : string, driver : ChromeDriver) =
    ...
    override this.Parsing() =
        ...
        if reader.HasRows then reader.Close()
            ...
            let IdOrg = ref 0
            let mutable inn = ""
            let PurName = driver.findElementWithoutException ("//h2/p")
            let CusName = driver.findElementWithoutException ("//div[. = 'Заказчик']/following-sibling::div/p")
            if CusName <> "" then 
            ...
            let idLot = ref 0
            let fin_source = ""
            let Nmck = (driver.findElementWithoutException ("//div[@class = 'text-control']//strong[contains(., '№')]/following-sibling::strong")).GetPriceFromString()
            let insertLot =
                sprintf 
            ...
            cmd12.Parameters.AddWithValue("@currency", "руб") |> ignore
            ...
            for po in purObjects do
                let name = po.findElementWithoutException ("./td[3]/div")
                let quantT = po.findElementWithoutException ("./td[6]/div")
                let quantP = quantT.Get2ListRegexp(@"(\d+)\s+-\s+(.+)")
                let quant = quantP.[0]
                let okei = quantP.[1]
                let okpd2F = po.findElementWithoutException ("./td[5]/div")
                let okpd2P = okpd2F.Get2ListRegexp(@"([\d.]+)\s+-\s+(.+)")
                let okpd2 = okpd2P.[0]
                let okpd2Name = okpd2P.[1]
                let insertLotitem =
                    sprintf 
                        "INSERT INTO %spurchase_object SET id_lot = @id_lot, name = @name, quantity_value = @quantity_value, customer_quantity_value = @customer_quantity_value, okei = @okei, okpd2_code = @okpd2_code, okpd_name = @okpd_name" 
                        stn.Prefix
                ...
                cmd19.Parameters.AddWithValue("@okpd_name", okpd2Name) |> ignore
