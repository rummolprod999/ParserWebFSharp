...
type TenderPetr(stn: Settings.T, tn: EshopRzdRec, typeFz: int, etpName: string, etpUrl: string, driver: ChromeDriver) =
    ...
    override this.Parsing() =
        ...
        if reader.HasRows then
            ...
            let regionS =
                Tools.GetRegionString(tn.RegionName)
            ...
            for doc in documents do
                ...
                | x ->
                    ...
                    cmd5.ExecuteNonQuery() |> ignore*)
            let purObjects =
                driver.findElementsWithoutException ("//div[@class = 'panel ng-scope panel-default']")

            for po in purObjects do
                let nameT =
                    po.findElementWithoutException (".//div[i[contains(., 'ОКПД2:')]]")

                let okpd2T =
                    po.findElementWithoutException (".//div[i[contains(., 'ОКПД2:')]]/i")

                let name = nameT.Replace(okpd2T, "").Trim()

                let okpd2 =
                    okpd2T.Replace("ОКПД2:", "").Trim()

                let sumOr =
                    po.findElementWithoutException (".//span[contains(@class, 'pull-right ')]")

                let listSum =
                    sumOr.Get4ListRegexp("^([\d. ]+)\s+₽\s+X\s+(\d+)\s(.+)\s=\s+([\d. ]+)\s+₽")
                ...
                cmd19.Parameters.AddWithValue("@sum", listSum.[3].RegexDeleteWhitespace())
                |> ignore

                cmd19.Parameters.AddWithValue("@price", listSum.[0].RegexDeleteWhitespace())
                |> ignore

                cmd19.Parameters.AddWithValue("@quantity_value", listSum.[1].RegexDeleteWhitespace())
                |> ignore

                cmd19.Parameters.AddWithValue("@customer_quantity_value", listSum.[1].RegexDeleteWhitespace())
                |> ignore

                cmd19.Parameters.AddWithValue("@okei", listSum.[2])
                |> ignore
