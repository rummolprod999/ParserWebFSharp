...
type ParserComitaZmo(stn : Settings.T) =
    ...
    let url = "https://etp.comita.ru/publicProceduresSOC"
    ...
    member private this.ParserTendersList (driver : ChromeDriver) (t : ComitaRec) =
        try 
            let T = TenderComitaZmo(set, t, 304, "АО «Комита» Закупки малого объема", "https://etp.comita.ru/publicProceduresSOC", driver)
            T.Parsing()
