...
type TenderKsk(stn: Settings.T, tn: KskRec, typeFz: int, etpName: string, etpUrl: string) =
    ...
    override this.Parsing() =
        ...
        let res =
            builder {
                ...
                match updated with
                | true -> incr TenderKsk.tenderUpCount
                | false -> incr TenderKsk.tenderCount
    ...
    member private this.GetAttachments(con: MySqlConnection, idTender: int) =
        for doc in tn.DocList do
            ...
            let url =
                sprintf "http://www.gt-ksk.com%s" urlDoc
