...
type TenderChPt(stn : Settings.T, tn : ChPtRec, typeFz : int, etpName : string, etpUrl : string) =
    ...
    override this.Parsing() =
        ...
        if reader.HasRows then reader.Close()
            ...
            let Printform = href
            let IdOrg = ref 0
            let OrgName = "ООО «Чебаркульская птица»"
            if OrgName <> "" then 
                ...
                | false -> 
                    ...
                    let addOrganizer =
                        sprintf 
                            ...
                            stn.Prefix
                    let contactPerson = ""
                    let postAddress = "ЧЕЛЯБИНСКАЯ ОБЛ.,ЧЕБАРКУЛЬСКИЙ Р-Н,П ТИМИРЯЗЕВСКИЙ,УЛ МИЧУРИНА, Д 3"
                    let factAddress = "ЧЕЛЯБИНСКАЯ ОБЛ.,ЧЕБАРКУЛЬСКИЙ Р-Н,П ТИМИРЯЗЕВСКИЙ,УЛ МИЧУРИНА, Д 3"
                    let phone = "8 (35168) 2-07-11, 8 (35168) 7-15-61"
                    let inn = "7420008157"
            ...
            let regionS = Tools.GetRegionString("челяб")
            if regionS <> "" then 
            ...
            match updated with
            | true -> incr TenderChPt.tenderUpCount
            | false -> incr TenderChPt.tenderCount
