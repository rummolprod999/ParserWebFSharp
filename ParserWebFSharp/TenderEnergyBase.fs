...
type TenderEnergyBase(stn: Settings.T, tn: EnergyBaseRec, typeFz: int, etpName: string, etpUrl: string) =
    ...
    override this.Parsing() =
        ...
        let res =
            builder {
                ...
                let Page =
                    Download.DownloadUseProxy(Settings.UseProxy, tn.Href)
                ...
                let contactPerson1 =
                    InlineHtmlNavigator nav "//div[. = 'ФИО']//following-sibling::div"

                let contactPerson2 =
                    InlineHtmlNavigator nav "//div[. = 'Электронная почта']//following-sibling::div"

                let contactPerson3 =
                    InlineHtmlNavigator nav "//div[. = 'Телефон']//following-sibling::div"
                ...
                cmd9.Parameters.AddWithValue("@scoring_date", tn.DateScoring)
                |> ignore
                ...
                cmd9.Parameters.AddWithValue("@notice_version", noticeVer.Trim())
                |> ignore
                ...
                let cusInn =
                    InlineHtmlNavigator nav "//span[. = 'ИНН/КПП']/.."

                let cusInn =
                    cusInn.Replace("ИНН/КПП", "").Trim()
    ...
    member private this.GetLots(con: MySqlConnection, idTender: int, idCustomer, doc: HtmlDocument) =
        ...
        let lots =
            doc.DocumentNode.SelectNodes("//div[. = 'Название лота']//following-sibling::div")
        ...
        if lots <> null then
            for l in lots do
                ...
                let delivPlace =
                    doc.DocumentNode.Gsn("//div[. = 'Место выполнения работ/услуг']//following-sibling::div")
    ...
    member private this.GetAttachments(con: MySqlConnection, idTender: int, doc: HtmlDocument) =
        let documents =
            doc.DocumentNode.SelectNodes("//div[@class = 'tender-attachments']//a")
