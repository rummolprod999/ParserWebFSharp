...
type ParserAsgor(stn: Settings.T) =
    ...
    let url =
        "https://etp.asgor.su/admin/TendProcUserSRO2.aspx?Frame=1"
    ...
    override this.Parsing() =
        ...
        | s ->
            ...
            let tens =
                documents.QuerySelectorAll("#proc-list div.proc")
    ...
    member private this.ParsingTender (t: IElement) (url: string) =
        ...
        let mutable Href =
            ...
            | None ->
                raise
                <| System.NullReferenceException(sprintf "Href not found in %s %s" url HrefT)
        ...
        let dateCons =
            ...
            >> (fun (s: string) -> s.ReplaceDateAsgor())

        let datePub =
            match (dateCons PubDateT)
                      .DateFromString("d MM yyyy HH:mm")
                with
            | Some d -> d
            | None ->
                raise
        ...
        let dateEnd =
            match (dateCons EndDateT)
                      .DateFromString("d MM yyyy HH:mm")
                with
            | Some d -> d
            | None ->
                raise
                <| System.Exception(sprintf "cannot parse dateEnd %s" EndDateT)
        ...
        let NameLotsT =
            t.QuerySelectorAll("div.lots div.lot")

        let mutable Lots = Set.empty

        NameLotsT
        |> Seq.iter (fun x -> Lots <- Lots.Add(x.TextContent.Trim()))
        ...
        try
            let T =
                TenderAsgor(set, ten, 72, "ООО \"АСГОР\"", "https://etp.asgor.su/")
