...
open TypeE
type TenderNeftisa(stn: Settings.T, tn: NeftisaRec, typeFz: int, etpName: string, etpUrl: string) =
    ...
    override this.Parsing() =
        ...
        let res =
                   builder {
                        ...
                        let! dateEndT = nav.GsnDoc "//span[. = 'Прием заявок до']/following-sibling::p"
                        let! dateEnd = dateEndT.DateFromStringDocMin("d.MM.yyyy")
                        ...
                        let idTender = ref 0
                        let numVersion = 1
                        let idRegion = this.GetReginId con tn.Region stn
                        ...
                        match updated with
                        | true -> incr TenderNeftisa.tenderUpCount
                        | false -> incr TenderNeftisa.tenderCount
    ...
    member private this.GetLots(con: MySqlConnection, idTender: int, idCustomer, doc: HtmlDocument) =
                ...
                let delivPlace = doc.DocumentNode.Gsn("//span[. = 'Адрес']/following-sibling::p")
                if delivPlace <> "" then
    ...
    member private this.GetAttachments(con: MySqlConnection, idTender: int, doc: HtmlDocument) =
        let documents = doc.DocumentNode.SelectNodes("//a[@class = 'docs-list__item']")
        if documents <> null then
            for d in documents do
                let docName = d.Gsn(".//p[1]")
                let docUrl = d.GsnAtr "." "href"
                if docUrl <> "" then
                    let docUrl = sprintf "https://www.neftisa.ru%s" docUrl
