...
type TenderBhm(stn: Settings.T, tn: BhmRec, typeFz: int, etpName: string, etpUrl: string) =
    ...
    override this.Parsing() =
        ...
        let res =
            builder {
                ...
                match updated with
                | true -> incr TenderBhm.tenderUpCount
                | false -> incr TenderBhm.tenderCount
    ...
    member private this.GetPos(con: MySqlConnection, idLot: int, idCustomer: int) =
        for p in tn.Products do
            ...
            cmd19.Parameters.AddWithValue("@name", p.Name)
            |> ignore
            ...
            cmd19.Parameters.AddWithValue("@quantity_value", p.Quantity)
            |> ignore

            cmd19.Parameters.AddWithValue("@customer_quantity_value", p.Quantity)
            |> ignore

            cmd19.Parameters.AddWithValue("@okei", p.Okei)
            |> ignore
