...
type ParserSamaraGips(stn: Settings.T) =
    ...
    member private __.ParsingPage(url: string) =
        ...
        | s ->
            ...
            let tens = documents.QuerySelectorAll("table.items--tenders tr.item--tender").ToList().Skip(1)
            for t in tens do
                    try
    ...
    member private __.ParsingTender (t: IElement) (url: string) =
        ...
        let res = builder {
            ...
            let href = sprintf "https://samaragips.ru%s" href
            let! datePubT = t.GsnDocWithError "td:nth-child(1)" <| sprintf "datePubT not found %s %s " url (t.TextContent)
            let datePub = datePubT.DateFromStringOrMin("dd.MM.yyyy")
            let! status = t.GsnDocWithError "td.item-status" <| sprintf "status not found %s %s " url (t.TextContent)
            let ten =
                { Href=href
                  PurNum = purNum
                  PurName=purName
                  DatePub=datePub
                  Status=status}
            let T = TenderSamaraGips(set, ten, 282, "ЗАО \"САМАРСКИЙ ГИПСОВЫЙ КОМБИНАТ\"", "https://samaragips.ru/")
            T.Parsing()
            return ""
        }
        match res with
