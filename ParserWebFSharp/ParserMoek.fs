...
type ParserMoek(stn: Settings.T) =
    ...
    member private this.ParserSelen(driver: ChromeDriver) =
        ...
        wait.Until (fun dr ->
            dr
                .FindElement(
                    By.XPath("//table[contains(@class, 'data')]/tbody/tr[1]")
                )
                .Displayed)
        |> ignore
    ...
    member private this.ParserTendersList (_: ChromeDriver) (t: MoekRec) =
        try
            let T =
                TenderMoek(set, t, 239, "ПАО «МОЭК»", "https://www.moek.ru/")
    ...
    member private this.ParserTenders(i: IWebElement) =
        ...
        let res =
            builder {
                ...
                let! purName = i.findElementWithoutException ("./td[2]/p[2]", sprintf "purName not found %s" i.Text)
                ...
                let! orgName = i.findElementWithoutException ("./td[4]/p", sprintf "orgName not found %s" i.Text)
                let! cusName = i.findElementWithoutException ("./td[3]/p", sprintf "cusName not found %s" i.Text)
                let! endDateT = i.findElementWithoutException ("./td[1]/p/span", sprintf "endDateT not found %s" i.Text)
