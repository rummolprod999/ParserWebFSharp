...
type TenderBarnaulTm(stn: Settings.T, tn: RosSelRec, typeFz: int, etpName: string, etpUrl: string, driver: ChromeDriver) =
        ...
        let res =
            builder {
                ...
                wait.Until (fun dr ->
                    dr
                        .FindElement(
                            By.XPath("//section[contains(., 'Дата начала')]")
                        )
                        .Displayed)
                |> ignore

                let body =
                    driver.FindElement(By.XPath("//section[contains(., 'Дата начала')]"))
                ...
                let! dateEndT1 =
                    body.Text.Get1Optional("Дата окончание приема тендерных заявок:\s+(\d{2}\.\d{2}\.\d{4})")
                ...
                match updated with
                | true -> incr TenderBarnaulTm.tenderUpCount
                | false -> incr TenderBarnaulTm.tenderCount
