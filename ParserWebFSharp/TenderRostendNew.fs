...
type TenderRosTendNew(stn : Settings.T, tn : RosTendRecNew, typeFz : int, etpName : string, etpUrl : string, noticeVer : string) =
    ...
    override this.Parsing() =
        ...
        if reader.HasRows then reader.Close()
            ...
            for row in dt.Rows do
                ...
                match tn.DateUpd >= ((row.["date_version"]) :?> DateTime) with
                | true -> row.["cancel"] <- 1
            ...
            cmd9.Parameters.AddWithValue("@date_version", tn.DateUpd) |> ignore
