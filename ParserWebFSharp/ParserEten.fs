...
type ParserEten(stn: Settings.T) =
    ...
    member __.Prepared(driver: ChromeDriver, wait: WebDriverWait) =
        ...
        driver
            .FindElement(By.XPath("//a[@id = 'langRu']"))
            .Click()

        wait.Until (fun dr ->
            dr
                .FindElement(
                    By.XPath("//table[@id = 'list']/tbody/tr[50]")
                )
                .Displayed)
        |> ignore
    ...
    member private this.ParserTendersList (driver: ChromeDriver) (t: EtenRec) =
        try
            let T =
                TenderEten(set, t, 210, "Регистр Государственных закупок", "http://etender.gov.md/", driver)
    ...
    member private this.ParserTenders(i: IWebElement) =
        ...
        let result =
            builder {
                let! purNum =
                    i.findElementWithoutException (".//td[2]", sprintf "purNum not found, inner text - %s" i.Text)
                ...
                let! pwName = i.findElementWithoutException (".//td[5]", sprintf "purName not found %s" i.Text)
                let! status = i.findElementWithoutException (".//td[8]", sprintf "status not found %s" i.Text)
                let! purName = i.findElementWithoutException (".//td[9]", sprintf "purName not found %s" i.Text)
