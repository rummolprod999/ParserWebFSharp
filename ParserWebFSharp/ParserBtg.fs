...
type ParserBtg(stn: Settings.T) =
    ...
    member private this.ParserSelen(driver: ChromeDriver) =
        ...
        wait.Until (fun dr ->
            dr
                .FindElement(
                    By.XPath("//div[contains(@class, 'dataTables_paginate')]/following-sibling::table/tbody")
                )
                .Displayed)
        |> ignore
    ...
    member private this.ParserTendersList (_: ChromeDriver) (t: BtgRec) =
        try
            let T =
                TenderBtg(set, t, 115, "ОАО «Газпром трансгаз Беларусь»", "http://www.btg.by")
    ...
    member private this.GetNextPage (driver: ChromeDriver) (wait: WebDriverWait) =
        for i in 2..2 do
            try
                ...
                this.Clicker driver
                <| sprintf "//span[@class = 'paginate_button' and . = '%d']" i
    ...
    member private this.ParserTenders (_: ChromeDriver) (t: IWebElement) =
        ...
        let result =
            builder {
                ...
                let hrefT =
                    t.FindElement(By.XPath(".//td[2]/p[1]/a"))
                ...
                let! dateEndT =
                    t.findElementWithoutException (".//td[1]/p/span[@class = 'tender_date']", "dateEndT not found")

                let! (dat, tm) =
                    dateEndT.Get2FromRegexpOptional("(\d{2}\.\d{2}\.\d{4}).+(\d{2}:\d{2})", "date and time not found")
