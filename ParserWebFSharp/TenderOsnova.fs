...
type TenderOsnova(stn: Settings.T, tn: OsnovaRec, typeFz: int, etpName: string, etpUrl: string) =
    ...
    member private this.GetPurchaseObjects(con: MySqlConnection, idCustomer, idLot) =
        ...
        if Page = "" || Page = null then failwith <| sprintf "PurchaseObjects page was not downloaded on href %s" urlPurObj
        let htmlDoc = HtmlDocument()
        htmlDoc.LoadHtml(Page)
        let nav = (htmlDoc.CreateNavigator()) :?> HtmlNodeNavigator
        let purObjects = nav.CurrentDocument.DocumentNode.SelectNodesOrEmpty("//tr[contains(@class, 'treegrid-parent') and not (contains(@class, 'expanded'))]")
