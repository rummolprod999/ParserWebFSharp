...
type Tender =
    ...
    override this.GetEtp (con: MySqlConnection) (stn: Settings.T) : int =
        let selectEtp =
            sprintf "SELECT id_etp FROM %setp WHERE name = @name AND url = @url" stn.Prefix

        let cmd6 = new MySqlCommand(selectEtp, con)
        cmd6.Prepare()

        cmd6.Parameters.AddWithValue("@name", this.EtpName)
        |> ignore

        cmd6.Parameters.AddWithValue("@url", this.EtpUrl)
        |> ignore
        ...
        | false ->
            ...
            let insertEtp =
                sprintf "INSERT INTO %setp SET name= @name, url= @url, conf=0" stn.Prefix

            let cmd7 = new MySqlCommand(insertEtp, con)
            cmd7.Prepare()

            cmd7.Parameters.AddWithValue("@name", this.etpName)
            |> ignore

            cmd7.Parameters.AddWithValue("@url", this.etpUrl)
            |> ignore
    ...
    override this.GetPlacingWay (con: MySqlConnection) (placingWayName: string) (stn: Settings.T) : int =
        ...
        let cmd6 =
            new MySqlCommand(selectPlacingWay, con)

        cmd6.Prepare()

        cmd6.Parameters.AddWithValue("@name", placingWayName)
        |> ignore
        ...
        | true ->
            ...
            let idPlacingWay =
                reader3.GetInt32("id_placing_way")
        ...
        | false ->
            ...
            let cmd7 =
                new MySqlCommand(insertPlacingWay, con)
            ...
            cmd7.Parameters.AddWithValue("@conformity", conf)
            |> ignore
    ...
    member this.GetReginId (con: MySqlConnection) (regionName: string) (stn: Settings.T) : int =
        let mutable idRegion = 0

        let regionS =
            Tools.GetRegionString(regionName)
    ...
    member this.AddVerNumber (con: MySqlConnection) (pn: string) (stn: Settings.T) (typeFz: int) : unit =
        ...
        let cmd1 =
            new MySqlCommand(selectTenders, con)

        cmd1.Prepare()

        cmd1.Parameters.AddWithValue("@purchaseNumber", pn)
        |> ignore
        ...
        if dt1.Rows.Count > 0 then
            ...
            for ten in dt1.Rows do
                ...
                let cmd2 =
                    new MySqlCommand(updateTender, con)
    ...
    member this.TenderKwords (con: MySqlConnection) (idTender: int) (stn: Settings.T) : unit =
        ...
        let cmd1 =
            new MySqlCommand(selectPurObj, con)
        ...
        if dt.Rows.Count > 0 then
            ...
            for row in distrDt do
                ...
                resString.Append(sprintf "%s %s " name okpdName)
                |> ignore

        let selectAttach =
            sprintf "SELECT DISTINCT file_name FROM %sattachment WHERE id_tender = @id_tender" stn.Prefix

        let cmd2 =
            new MySqlCommand(selectAttach, con)
        ...
        let cmd3 =
            new MySqlCommand(selectPurInf, con)
        ...
        if dt3.Rows.Count > 0 then
            for row in dt3.Rows do
                ...
                idOrg
                := match row.IsNull("id_organizer") with
                   | true -> 0
        ...
        | true ->
            ...
            if dt4.Rows.Count > 0 then
                for row in dt4.Rows do
                    ...
                    resString.Append(sprintf " %s %s" innOrg nameOrg)
                    |> ignore
        | false -> ()
        ...
        if dt5.Rows.Count > 0 then
            ...
            for row in distrDt do
                ...
                resString.Append(sprintf " %s %s" innC fullNameC)
                |> ignore

        let mutable resS =
            Regex.Replace(resString.ToString(), @"\s+", " ")
        ...
        cmd5.Parameters.AddWithValue("@tender_kwords", resS)
        |> ignore

        let res = cmd5.ExecuteNonQuery()

        if res <> 1 then
            Logging.Log.logger ("Не удалось обновить tender_kwords", idTender)
