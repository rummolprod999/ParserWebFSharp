    ...
    override this.Parsing() =
        ...
        let res =
                   builder {
                        ...
                        let IdOrg = ref 0
                        let! cusName = body.findElementWithoutExceptionOptional("//td[contains(., 'Предприятие-инициатор:')]/following-sibling::td", sprintf "cusName not found %s" tn.Href)
                        if cusName <> "" then
                            ...
                            | false ->
                                ...
                                let! contactPerson = body.findElementWithoutExceptionOptional("//td[contains(., 'Ответственный за лот:')]/following-sibling::td", sprintf "contactPerson not found %s" tn.Href)
                                let! postAddress = body.findElementWithoutExceptionOptional("//td[contains(., 'Адрес:')]/following-sibling::td", sprintf "postAddress not found %s" tn.Href)
                                let factAddress = ""
                                let! phone = body.findElementWithoutExceptionOptional("//td[contains(., 'Телефоны:')]/following-sibling::td", sprintf "phone not found %s" tn.Href)
                                let inn = ""
                                let! email = body.findElementWithoutExceptionOptional("//td[contains(., 'Email ответственного:')]/following-sibling::td/a", sprintf "email not found %s" tn.Href)
                                let cmd5 = new MySqlCommand(addOrganizer, con)
                                cmd5.Parameters.AddWithValue("@full_name", cusName) |> ignore
                        ...
                        match updated with
                        | true -> incr TenderMedic.tenderUpCount
                        | false -> incr TenderMedic.tenderCount
                        let attachments = body.findElementsWithoutException("//td[contains(., 'Документы:')]/following-sibling::td/a")
                        ...
                        let purObjects = body.findElementsWithoutException("//table[@class = 't_lot_table' and contains(., 'Спецификации')]/tbody/tr[position() > 1]")
