...
type ParserDomRu(stn: Settings.T) =
    ...
    let urls = [| "https://zakupki.ertelecom.ru/" |]
    ...
    member private __.ParsingPage(url: string) =
        ...
        | s ->
            ...
            let tens =
                documents
                    .QuerySelectorAll("table.contractor_search_table tr[id^='For_']")
                    .ToList()
    ...
    member private __.ParsingTender (t: IElement) (url: string) =
        ...
        let res =
            builder {
                ...
                let href =
                    sprintf "https://zakupki.domru.ru%s" href
                ...
                let! datePubT =
                    t.GsnDocWithError "td:nth-child(4)"
                    <| sprintf "datePubT not found %s %s " url (t.TextContent)

                let datePubT =
                    datePubT.RegexReplace().ReplaceDateAriba()

                let datePub =
                    datePubT.DateFromStringOrMin("dd.MM.yyyy")

                let! dateEndT =
                    t.GsnDocWithError "td:nth-child(5)"
                    <| sprintf "dateEndT not found %s %s " url (t.TextContent)

                let dateEndT =
                    dateEndT.RegexReplace().ReplaceDateAriba()
                ...
                let T =
                    TenderDomRu(set, ten, 280, "Портал закупок Дом.ru", "https://zakupki.domru.ru/")
