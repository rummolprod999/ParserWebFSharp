...
type Init(s: Settings.T, arg: Arguments) =
    ...
    do
        if
            String.IsNullOrEmpty(s.TempPathTenders)
            || String.IsNullOrEmpty(s.LogPathTenders)
        then
            ...
            | true ->
                let dirInfo =
                    DirectoryInfo(s.TempPathTenders)

                dirInfo.Delete(true)

                Directory.CreateDirectory(s.TempPathTenders)
                |> ignore
            | false ->
                Directory.CreateDirectory(s.TempPathTenders)
                |> ignore
            ...
            | false ->
                Directory.CreateDirectory(s.LogPathTenders)
                |> ignore
            | true -> ()
    ...
    member private this.ParsingRossel() =
        ...
        try
            this.GetParser(ParserRosselNoSelen(s))
        with
            | ex -> Logging.Log.logger ex
        ...
        Logging.Log.logger (
            sprintf "Обновили ПАО «Ростелеком» и подведомственных организаций тендеров %d" !TenderRossel.tenderUpCountRt
        )
        ...
        Logging.Log.logger (sprintf "Добавили бизнес тендеров %d" !TenderRossel.tenderCountBus)
        Logging.Log.logger (sprintf "Обновили бизнес тендеров %d" !TenderRossel.tenderUpCountBus)
        Logging.Log.logger (sprintf "Добавили не попавшие в категории тендеров %d" !TenderRossel.tenderAllCount)
        Logging.Log.logger (sprintf "Обновили не попавшие в категории тендеров %d" !TenderRossel.tenderUpAllCount)
    ...
    member private this.ParsingSevZakaz() =
        Logging.Log.logger "Начало парсинга"

        try
            this.GetParser(ParserSevZakaz(s))
        with
            | ex -> Logging.Log.logger ex

        Logging.Log.logger "Конец парсинга"
        Logging.Log.logger (sprintf "Добавили тендеров %d" !TenderSevZakaz.tenderCount)
        Logging.Log.logger (sprintf "Обновили тендеров %d" !TenderSevZakaz.tenderUpCount)
    
    member private this.ParsingDfSamara() =
        Logging.Log.logger "Начало парсинга"

        try
            this.GetParser(ParserDfSamara(s))
        with
            | ex -> Logging.Log.logger ex
