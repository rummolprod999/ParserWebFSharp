...
type ParserRtCi(stn: Settings.T) =
    ...
    let url =
        "https://zakupki.rt-ci.ru/procurement/?PAGEN_1="
        ...
        | s ->
            ...
            let tens =
                nav
                    .CurrentDocument
                    .DocumentNode
                    .SelectNodesOrEmpty("//a[@class = 'card-procurement-info']")
                    .ToList()
    ...
    member private __.ParsingTender (t: HtmlNode) (_: string) =
        ...
        let res =
            builder {
                let! purName =
                    t.GsnDocWithError ".//span[. = 'Предмет закупки']/following-sibling::span"
                    <| sprintf "purName not found %s %s " url (t.InnerText)

                let! hrefT =
                    t.GsnAtrDocWithError "."
                    <| "href"
                ...
                let! pwName =
                    t.GsnDocWithError ".//div[@class = 'h4 card-procurement-info__head']"
                    <| sprintf "pwName not found %s %s " url (t.InnerText)

                let! purNum =
                    t.GsnDocWithError ".//div[@class = 'card-procurement-info__number']"
                    <| sprintf "purNum not found %s %s " url (t.InnerText)

                let purNum = purNum.Replace("№", "").Trim()

                let! datePubT =
                    t.GsnDocWithError ".//span[. = 'Дата размещения']/following-sibling::span"
                ...
                let T =
                    TenderRtCi(set, tend, 295, "РТ-Комплектимпекс", "https://zakupki.rt-ci.ru/")
