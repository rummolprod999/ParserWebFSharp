...
type ParserVolgZmo(stn: Settings.T) =
    ...
    member private __.ParsingPage(url: string) =
        ...
        | s ->
            ...
            let tens = documents.QuerySelectorAll("table tbody tr.results-row").ToList()
            for t in tens do
                    try
    ...
    member private __.ParsingTender (t: IElement) (url: string) =
        ...
        let res = builder {
            ...
            let! purName = t.GsnDocWithError "td:nth-of-type(3)" <| sprintf "purName not found %s %s " url (t.TextContent)
            let! cusName = t.GsnDocWithError "td:nth-of-type(4)" <| sprintf "cusName not found %s %s " url (t.TextContent)
            ...
            let! datePubT = t.GsnDocWithError "td:nth-of-type(6)" <| sprintf "datePubT not found %s %s " url (t.TextContent)
            let! datePub = datePubT.DateFromStringDoc ("dd.MM.yyyy", sprintf "datePub not found %s %s " href datePubT)
            let! endDateT = t.GsnDocWithError "td:nth-of-type(7)" <| sprintf "endDateT not found %s %s " url (t.TextContent)
            let! dateEnd = endDateT.DateFromStringDoc ("dd.MM.yyyy HH:mm", sprintf "dateEnd not found %s %s " href endDateT)
            let! status = t.GsnAtrDoc "td:nth-of-type(8)" <| sprintf "status not found %s %s " url (t.TextContent)
            let tend = {  Href = href
                          DateEnd = dateEnd
                          DatePub = datePub
                          PurNum = purNum
                          ...
                          Nmck = nmck
                          Status = status}
            let T = TenderVolgZmo(set, tend, 234, "Комитет Государственного заказа Вологодской области", "https://szvo.gov35.ru/")
            T.Parsing()
            return ""
        }
        match res with
