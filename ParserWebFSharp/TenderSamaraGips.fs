...
type TenderSamaraGips(stn: Settings.T, tn: SamaraGipsRec, typeFz: int, etpName: string, etpUrl: string) =
    ...
    override this.Parsing() =
        ...
        let res =
                   builder {
                        ...
                        let dateEndT = InlineHtmlNavigator nav "//p[strong[contains(., 'Предложения принимаются до')]]"
                        let dateEndT = dateEndT.Replace("Предложения принимаются до", "").Replace(":", "").Trim().RegexCutWhitespace()
                        let! dateEnd = dateEndT.DateFromStringDoc ("HH-mm dd.MM.yyyy", sprintf "dateEnd not found %s %s " tn.Href dateEndT)
                        con.Open()
                        let selectTend =
                            sprintf 
                        ...
                        if orgName <> "" then
                            ...
                            | false ->
                                ...
                                let postAddress = ""
                                let factAddress = ""
                                let phone = "+7 (846) 269-64-25 (+ 4166)"
                                let inn = ""
                                let email = "karyasova@samaragips.ru"
                        ...
                        match updated with
                        | true -> incr TenderSamaraGips.tenderUpCount
                        | false -> incr TenderSamaraGips.tenderCount
    ...
    member private this.GetLots(con: MySqlConnection, idTender: int, doc: HtmlDocument, idCustomer) =
        ...
        cmd12.Parameters.AddWithValue("@lot_name", "") |> ignore
        ...
        let delivPlace = ""
        let delivTerm1 = InlineHtmlNavigator nav "//p[strong[contains(., 'Условия оплаты')]]"
        let delivTerm2 = InlineHtmlNavigator nav "//p[strong[contains(., 'Особые условия')]]"
    ...
    member private this.GetAttachments(con: MySqlConnection, idTender: int, doc: HtmlDocument) =
        let documents = doc.DocumentNode.SelectNodes("//a[contains(@class, 'item item--small item--document')]")
        if documents <> null then
            let documents = documents.Skip(1)
            for d in documents do
                let docName = d.Gsn(".//div[@class = 'item-title']")
                let docUrl = d.GsnAtr "." "href"
                let docUrl = sprintf "https://samaragips.ru%s" docUrl
                if docUrl <> "" then
