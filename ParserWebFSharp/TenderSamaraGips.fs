...
type TenderSamaraGips(stn: Settings.T, tn: SamaraGipsRec, typeFz: int, etpName: string, etpUrl: string) =
    ...
    override this.Parsing() =
        ...
        let res =
            builder {
                ...
                let dateEndT =
                    InlineHtmlNavigator nav "//p[strong[contains(., 'Предложения принимаются до')]]"

                let dateEndT =
                    dateEndT
                        .Replace("Предложения принимаются до", "")
                        .Replace(":", "")
                        .Trim()
                        .RegexCutWhitespace()

                let! dateEnd =
                    dateEndT.DateFromStringDoc("HH-mm dd.MM.yyyy", sprintf "dateEnd not found %s %s " tn.Href dateEndT)
    ...
    member private this.GetLots(con: MySqlConnection, idTender: int, doc: HtmlDocument, idCustomer) =
        ...
        let delivTerm1 =
            InlineHtmlNavigator nav "//p[strong[contains(., 'Условия оплаты')]]"

        let delivTerm2 =
            InlineHtmlNavigator nav "//p[strong[contains(., 'Особые условия')]]"
    ...
    member private this.GetAttachments(con: MySqlConnection, idTender: int, doc: HtmlDocument) =
        let documents =
            doc.DocumentNode.SelectNodes("//a[contains(@class, 'item item--small item--document')]")
        ...
        if documents <> null then
            ...
            for d in documents do
                let docName =
                    d.Gsn(".//div[@class = 'item-title']")

                let docUrl = d.GsnAtr "." "href"

                let docUrl =
                    sprintf "https://samaragips.ru%s" docUrl
