...
type TenderRosAgro(stn: Settings.T, tn: RosAgroRec, typeFz: int, etpName: string, etpUrl: string) =
        ...
        let res =
            builder {
                ...
                if ogrName <> "" then
                    ...
                    cmd3.Parameters.AddWithValue("@full_name", ogrName)
                    |> ignore
                    ...
                    | false ->
                        ...
                        let factAddress =
                            "Россия, 125124, Москва, ул. Правды, д.26"
                        ...
                        cmd5.Parameters.AddWithValue("@full_name", ogrName)
                        |> ignore
    ...
    member private this.GetLots(con: MySqlConnection, idTender: int, doc: HtmlDocument) =
        ...
        let delivPlace =
            nav.Gsn "//td[contains(., 'Место поставки предмета закупки')]/following-sibling::td/htmlbox"

        let delivTerm =
            nav.Gsn "//td[contains(., 'Срок поставки предмета закупки')]/following-sibling::td/htmlbox"
    ...
    member private this.GetAttachments(con: MySqlConnection, idTender: int, doc: HtmlDocument) =
        ...
        let docs =
            doc.DocumentNode.SelectNodes("//tr[contains(., 'Документы')]//button")
        ...
        | _ ->
            for doc in docs do
                try
                    ...
                    let docUrl =
                        match docUrl.Get1FromRegexp "window\.location\.replace\('(.+)'\)" with
                        | None -> ""
                        | Some e -> e
