...
type ParserBelorusNeft(stn: Settings.T) =
    ...
    let url =
        "http://www.belorusneft-siberia.ru/sitesib/ru/addUp/purchases/current/"
        ...
        | s ->
            ...
            let tens =
                nav
                    .CurrentDocument
                    .DocumentNode
                    .SelectNodesOrEmpty("//div[@class = 'b-tenders__item']")
                    .ToList()
    ...
    member private __.ParsingTender (t: HtmlNode) (url: string) =
        ...
        let res =
            builder {
                let! purName =
                    t.GsnDocWithError ".//div[contains(@class, 'b-tenders__item_top')]"
                    <| sprintf "purName not found %s %s " url (t.InnerText)

                let! hrefT =
                    t.GsnAtrDocWithError ".//a[contains(@class, 'b-tenders__item_file')]"
                    <| "href"
                    <| sprintf "hrefT not found %s %s " url (t.InnerText)

                let href =
                    sprintf "http://www.belorusneft-siberia.ru%s" hrefT

                let purNum = Tools.createMD5 href

                let! datePubT =
                    t.GsnDocWithError ".//div[contains(@class, 'b-tenders__item_beginning')]/span"
                ...
                let dateEnd =
                    match dateEndT.DateFromStringOrMin("dd.MM.yyyy HH:mm") with
                ...
                let T =
                    TenderBelorusNeft(set, tend, 330, "ООО «Белоруснефть-Сибирь»", "http://www.belorusneft-siberia.ru/")
