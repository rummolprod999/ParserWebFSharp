...
type ParserForScience(stn: Settings.T) =
    ...
    override __.Parsing() =
        for i in pageC .. -1 .. 1 do
            try
                let url =
                    sprintf
                        "https://4science.ru/finsupports?view=list&page=%d&sortField=firstPublished&sortOrder=desc"
                        i
    ...
    member private __.ParsingPage(url: string) =
        ...
        | s ->
            ...
            let tens =
                documents
                    .QuerySelectorAll("div#fin-supports-list-inner > a.fin-list")
                    .ToList()
    ...
    member private __.ParsingTender (t: IElement) (url: string) =
        ...
        let res =
            builder {
                let! href =
                    t.GsnAtrSelfDocWithError "href"
                    <| sprintf "href not found %s %s " url (t.TextContent)

                let href =
                    sprintf "https://4science.ru%s" href

                let! purName =
                    t.GsnDocWithError "div.fin-list-title"
                    <| sprintf "purName not found %s %s " url (t.TextContent)

                let! pwName =
                    t.GsnDocWithError "div.fin-list-label"
                    <| sprintf "pwName not found %s %s " url (t.TextContent)

                let! datePubT =
                    t.GsnDocWithError "div.fin-list-title span.fin-list-date"
                    <| sprintf "datePubT not found %s %s " url (t.TextContent)

                let purName =
                    purName.Replace(datePubT, "").Trim()
                ...
                let! orgName =
                    t.GsnDocWithError "div.fin-list-info-in-last"
                    <| sprintf "orgName not found %s %s " url (t.TextContent)
                ...
                let T =
                    TenderForScience(set, tend, 233, "4science", "https://4science.ru/")
