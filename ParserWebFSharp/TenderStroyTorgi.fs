...
type TenderStroyTorgi(stn: Settings.T, tn: StroyTorgiRec, typeFz: int, etpName: string, etpUrl: string) =
    ...
    override this.Parsing() =
        ...
        if tn.Status.Contains("Завершены") then
        ...
        if reader.HasRows then
            ...
            | "" ->
                raise
                <| Exception(sprintf "cannot download page %s" tn.Url)
            | _ -> ()
            ...
            if not <| tn.OrgName.Contains("Закрытые торги")
               && tn.OrgName <> "" then
            ...
            let lots =
                doc.QuerySelectorAll("ul.lot-goods li ~ li")
