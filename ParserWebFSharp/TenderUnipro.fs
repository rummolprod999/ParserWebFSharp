...
type TenderUnipro(stn: Settings.T, tn: YmzRec, typeFz: int, etpName: string, etpUrl: string) =
    ...
    override this.Parsing() =
        ...
        let res =
                   builder {
                        ...
                        let IdOrg = ref 0
                        let orgName = InlineHtmlNavigator nav "//p[contains(., 'ОРГАНИЗАТОР:')]"
                        let orgName = HttpUtility.HtmlDecode(orgName).Replace("ОРГАНИЗАТОР:", "").Trim()
                        if orgName <> "" then
                            ...
                            | false ->
                                ...
                                let contactPerson = ""
                                let postAddress = InlineHtmlNavigator nav "//p[contains(., 'ПОЧТОВЫЙ АДРЕС:')]"
                                let postAddress = HttpUtility.HtmlDecode(postAddress).Replace("ПОЧТОВЫЙ АДРЕС:", "").Trim()
                                let factAddress = ""
                                let phone = ""
                                let inn = ""
                                let email = ""
                        ...
                        let pwName = InlineHtmlNavigator nav "//p[contains(., 'ФОРМА И СПОСОБ ЗАКУПКИ:')]"
                        let pwName = HttpUtility.HtmlDecode(pwName).Replace("ФОРМА И СПОСОБ ЗАКУПКИ:", "").Trim()
                        match pwName with
                        | "" -> ()
                        ...
                        match updated with
                        | true -> incr TenderUnipro.tenderUpCount
                        | false -> incr TenderUnipro.tenderCount
    ...
    member private this.GetLots(con: MySqlConnection, idTender: int, doc: HtmlDocument) =
        ...
        let cusName = InlineHtmlNavigator nav "//p[contains(., 'ЗАКАЗЧИК (и):')]"
        let cusName = HttpUtility.HtmlDecode(cusName).Replace("ЗАКАЗЧИК (и):", "").Trim()
        ...
        let delivTerm = ""
        let delivPlace  = InlineHtmlNavigator nav "//p[contains(., 'АДРЕС МЕСТА ОКАЗАНИЯ УСЛУГ:')]"
        let delivPlace = HttpUtility.HtmlDecode(delivPlace).Replace("АДРЕС МЕСТА ОКАЗАНИЯ УСЛУГ:", "").Trim()
    ...
    member private this.GetAttachments(con: MySqlConnection, idTender: int, doc: HtmlDocument) =
            ...
            let docs = doc.DocumentNode.SelectNodes("//a[contains(@href, '/files/')]")
            match docs with
            | null -> ()
            | _ -> 
                for doc in docs do
                        ...
                        if docName <> "" && docUrl <> "" then 
                            let d =
                                { name = docName.RegexCutWhitespace()
                                  url = sprintf "http://unipro.energy%s" docUrl }
                            docList.Add(d)
