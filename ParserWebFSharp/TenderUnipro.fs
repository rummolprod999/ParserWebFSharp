...
type TenderUnipro(stn: Settings.T, tn: YmzRec, typeFz: int, etpName: string, etpUrl: string) =
    ...
    override this.Parsing() =
        ...
        let res =
            builder {
                ...
                let orgName =
                    InlineHtmlNavigator nav "//p[contains(., 'ОРГАНИЗАТОР:')]"
                ...
                if orgName <> "" then
                    ...
                    | false ->
                        ...
                        let postAddress =
                            InlineHtmlNavigator nav "//p[contains(., 'ПОЧТОВЫЙ АДРЕС:')]"

                        let postAddress =
                            HttpUtility
                                .HtmlDecode(postAddress)
                                .Replace("ПОЧТОВЫЙ АДРЕС:", "")
                                .Trim()
                ...
                let pwName =
                    InlineHtmlNavigator nav "//p[contains(., 'ФОРМА И СПОСОБ ЗАКУПКИ:')]"

                let pwName =
                    HttpUtility
                        .HtmlDecode(pwName)
                        .Replace("ФОРМА И СПОСОБ ЗАКУПКИ:", "")
                        .Trim()
    ...
    member private this.GetLots(con: MySqlConnection, idTender: int, doc: HtmlDocument) =
        ...
        let cusName =
            InlineHtmlNavigator nav "//p[contains(., 'ЗАКАЗЧИК (и):')]"
        ...
        let delivPlace =
            InlineHtmlNavigator nav "//p[contains(., 'АДРЕС МЕСТА ОКАЗАНИЯ УСЛУГ:')]"

        let delivPlace =
            HttpUtility
                .HtmlDecode(delivPlace)
                .Replace("АДРЕС МЕСТА ОКАЗАНИЯ УСЛУГ:", "")
                .Trim()
    ...
    member private this.GetAttachments(con: MySqlConnection, idTender: int, doc: HtmlDocument) =
        ...
        let docs =
            doc.DocumentNode.SelectNodes("//a[contains(@href, '/files/')]")
