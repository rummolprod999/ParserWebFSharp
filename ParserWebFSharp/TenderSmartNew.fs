...
type TenderSmartNew(stn: Settings.T, tn: SmartRecNew, typeFz: int, etpName: string, etpUrl: string) =
    ...
    override this.Parsing() =
        ...
        let res =
            builder {
                ...
                let href =
                    sprintf "https://smarttender.biz/komertsiyni-torgy/%d/" tn.Id
                ...
                let jsonFromPageOptional =
                    Page.Get1FromRegexpDotAll "window.preloadedData = {\s+model:\s+(.+),\s+Resources:\s+{"

                let json =
                    match jsonFromPageOptional with
                    | None -> "{}"
                    | Some x -> x
    ...
    member private this.Docs(con: MySqlConnection, idTender: int, j) =
        try
            let docs =
                j.SelectTokens("Attachments.TENПРГ.*")

            for d in docs do
                let docName =
                    GetStringFromJtoken d "..AttachmentName"

                let docUrl =
                    GetStringFromJtoken d "..DownloadLink"
    ...
    member private this.PurObjs(con: MySqlConnection, idLot: int, idCus, j) =
        try
            ...
            for p in purObjs do
                ...
                let quant =
                    GetStringFromJtoken p "..Quantity"

                let price =
                    GetStringFromJtoken p "..Budget.Amount"
