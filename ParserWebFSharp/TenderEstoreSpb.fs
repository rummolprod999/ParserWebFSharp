...
type TenderEstoreSpb(stn: Settings.T, tn: EstoreSpbRec, typeFz: int, etpName: string, etpUrl: string) =
                   ...
                   builder {
                        ...
                        if tn.OrgName <> "" then
                            ...
                            | false ->
                                ...
                                let contactPerson = ""
                                let postAddress = InlineHtmlNavigator nav "//label[contains(., 'Почтовый адрес:')]/following-sibling::div/div"
                                let factAddress = InlineHtmlNavigator nav "//label[contains(., 'Местонахождение:')]/following-sibling::div/div"
                                let phone = InlineHtmlNavigator nav "//label[contains(., 'Контактный телефон')]/following-sibling::div/div"
                                let inn = ""
                                let email = InlineHtmlNavigator nav "//label[contains(., 'Электронная почта')]/following-sibling::div/div"
                                ...
                                ()
                        let idXml = InlineHtmlNavigator nav "//label[contains(., 'Идентификационный код закупки:')]/following-sibling::div/div"
                        ...
                        cmd9.Prepare()
                        cmd9.Parameters.AddWithValue("@id_xml", idXml) |> ignore
                        ...
                        match updated with
                        | true -> incr TenderEstoreSpb.tenderUpCount
                        | false -> incr TenderEstoreSpb.tenderCount
    ...
    member private this.GetLots(con: MySqlConnection, idTender: int, doc: HtmlDocument) =
        ...
        let finSource = nav.Gsn "//label[contains(., 'Источник финансирования:')]/following-sibling::div/div"
        let idLot = ref 0
        ...
        let purObjs = doc.DocumentNode.SelectNodes("//table[@id = 'items']/tbody/tr[@class = 'lotItem']")
        match purObjs with
        | null -> ()
        | _ -> 
            for po in purObjs do
                ...
                let name = po.Gsn "./td[11]"
                let price = (po.Gsn "./td[5]").GetPriceFromStringKz()
                let sum = (po.Gsn "./td[6]").GetPriceFromStringKz()
                let okpd2 = po.Gsn "./td[7]"
                let name = (sprintf "%s %s" name okpd2).Trim()
                ...
                cmd19.Parameters.AddWithValue("@okpd_name", name) |> ignore
                cmd19.ExecuteNonQuery() |> ignore
        let delivPlace = nav.Gsn "//label[contains(., 'Место поставки товаров, выполнения работ, оказания услуг:')]/following-sibling::div/div"
        let delivTerm = nav.Gsn "//label[contains(., 'Срок доставки товаров, выполнения работ, оказания услуг:')]/following-sibling::div/div"
