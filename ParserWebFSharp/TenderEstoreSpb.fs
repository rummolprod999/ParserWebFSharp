...
type TenderEstoreSpb(stn: Settings.T, tn: EstoreSpbRec, typeFz: int, etpName: string, etpUrl: string) =
            ...
            builder {
                ...
                if tn.OrgName <> "" then
                    ...
                    | false ->
                        ...
                        let postAddress =
                            InlineHtmlNavigator nav "//label[contains(., 'Почтовый адрес:')]/following-sibling::div/div"

                        let factAddress =
                            InlineHtmlNavigator
                                nav
                                "//label[contains(., 'Местонахождение:')]/following-sibling::div/div"

                        let phone =
                            InlineHtmlNavigator
                                nav
                                "//label[contains(., 'Контактный телефон')]/following-sibling::div/div"
                        ...
                        let email =
                            InlineHtmlNavigator
                                nav
                                "//label[contains(., 'Электронная почта')]/following-sibling::div/div"
                ...
                let idXml =
                    InlineHtmlNavigator
                        nav
                        "//label[contains(., 'Идентификационный код закупки:')]/following-sibling::div/div"
                ...
                cmd9.Parameters.AddWithValue("@id_xml", idXml)
                |> ignore
    ...
    member private this.GetLots(con: MySqlConnection, idTender: int, doc: HtmlDocument) =
        ...
        let finSource =
            nav.Gsn "//label[contains(., 'Источник финансирования:')]/following-sibling::div/div"
        ...
        let purObjs =
            doc.DocumentNode.SelectNodes("//table[@id = 'items']/tbody/tr[@class = 'lotItem']")
        ...
        | _ ->
            for po in purObjs do
                ...
                cmd19.Parameters.AddWithValue("@okpd_name", name)
                |> ignore
        ...
        let delivPlace =
            nav.Gsn
                "//label[contains(., 'Место поставки товаров, выполнения работ, оказания услуг:')]/following-sibling::div/div"

        let delivTerm =
            nav.Gsn
                "//label[contains(., 'Срок доставки товаров, выполнения работ, оказания услуг:')]/following-sibling::div/div"
    ...
    member private this.GetAttachments(con: MySqlConnection, idTender: int, doc: HtmlDocument) =
        ...
        let docs =
            doc.DocumentNode.SelectNodes("//table[@id = 'documents']/tbody/tr//a")
        ...
        let docs =
            doc.DocumentNode.SelectNodes("//button[@name = 'submit_viewStandartContract']")
        ...
        | _ ->
            for doc in docs do
                try
                    ...
                    let docUrl =
                        doc.getAttrWithoutException ("onclick")
