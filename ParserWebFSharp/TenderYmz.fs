...
type TenderYmz(stn: Settings.T, tn: YmzRec, typeFz: int, etpName: string, etpUrl: string) =
    ...
    override this.Parsing() =
        ...
        let res =
            builder {
                ...
                match updated with
                | true -> incr TenderYmz.tenderUpCount
                | false -> incr TenderYmz.tenderCount
    ...
    member private this.GetAttachments(con: MySqlConnection, idTender: int, doc: HtmlDocument) =
        ...
        let docs =
            doc.DocumentNode.SelectNodes("//a[@class = 'document-item']")
                    ...
                    if docName <> "" && docUrl <> "" then
                        let d =
                            { name = docName.RegexCutWhitespace()
                              url = sprintf "https://www.ymzmotor.ru%s" docUrl }
