...
type TenderTenderer(stn: Settings.T, tn: TendererRec, typeFz: int, etpName: string, etpUrl: string) =
    ...
    override this.Parsing() =
        ...
        let selectTend =
            sprintf
                "SELECT id_tender FROM %stender WHERE purchase_number = @purchase_number AND type_fz = @type_fz AND end_date = @end_date AND doc_publish_date = @doc_publish_date AND bidding_date = @bidding_date"
                stn.Prefix
        ...
        cmd.Parameters.AddWithValue("@bidding_date", tn.DateBidding)
        |> ignore
        ...
        if reader.HasRows then
            ...
            cmd9.Parameters.AddWithValue("@bidding_date", tn.DateBidding)
            |> ignore
            ...
            match updated with
            | true -> incr TenderTenderer.tenderUpCount
            | false -> incr TenderTenderer.tenderCount
