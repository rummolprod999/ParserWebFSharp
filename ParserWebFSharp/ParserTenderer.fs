...
type ParserTenderer(stn: Settings.T) =
    ...
    member private this.GetPathList stpage =
        let Page =
            Download.DownloadString1251CookiesTenderer stpage

        match Page with
        | null
        | "" -> Logging.Log.logger ("Dont get start page", stpage)
        | s ->
            let parser = HtmlParser()
            let documents = parser.Parse(s)

            let tensurl =
                documents.QuerySelectorAll("div.page_tenders li a[href $='index.html']")

            tensurl
            |> Seq.iter (fun x ->
                let el = x.GetAttribute("href").Trim()

                if el.Contains("http://") then
                    listPathTenders.Add(el)
                else
                    listPathTenders.Add(sprintf "http://www.tenderer.ru%s" el))
    ...
    member private this.ParserUrl url =
        ...
        for i in 2..pageC do
            try
                let urlT =
                    url.Replace("index.html", sprintf "index%d.html" i)
    ...
    member private this.NextPage url : bool =
        let Page =
            Download.DownloadString1251CookiesTenderer url
        ...
        | s ->
            ...
            let mutable tens =
                documents.QuerySelectorAll("div.tender_table ul li")

            if tens.Length < 2 then
                false
            else
    ...
    member private this.ParserPage url =
        ...
        | s ->
            ...
            let tens =
                documents
                    .QuerySelectorAll("div.tender_table ul li")
                    .Skip(1)
    ...
    member private this.ParserTender (t: IElement) (url: string) =
        ...
        let purName =
            ...
            | null ->
                raise
                <| NullReferenceException(sprintf "PurName not found in %s" href)
        ...
        let purNum =
            ...
            | null ->
                raise
                <| NullReferenceException(sprintf "PurNum not found in %s" href)
            | ur -> ur.TextContent.Trim()
        ...
        let pubDateT =
            ...
            | null ->
                raise
                <| NullReferenceException(sprintf "pubDateT not found in %s" href)
            | ur -> ur.TextContent.Trim()
        ...
        try
            let T =
                TenderTenderer(set, ten, 129, "TENDERER.RU", "http://www.tenderer.ru")
