...
type ParserChPt(stn : Settings.T) =
    ...
    override this.Parsing() =
        ...
        | s -> 
            ...
            let tens = documents.QuerySelectorAll("ul.groups_list li:not(.head)")
            for t in tens do
                try 
    ...
    member private this.ParsingTender (t : IElement) (url : string) =
        ...
        let Href = sprintf "https://tp.chpt.ru%s" HrefT
        ...
        let ten : ChPtRec =
            { Href = Href
              ...
              Currency = Currency }
        try 
            let T = TenderChPt(set, ten, 84, "ООО «Чебаркульская птица»", "https://tp.chpt.ru/")
            T.Parsing()
