...
type ParserKg(stn: Settings.T) =
    ...
    let url =
        "http://zakupki.gov.kg/popp/view/order/list.xhtml"
    ...
    member private this.PreparedPage(driver: ChromeDriver) =
        ...
        try
            ...
            this.Clicker driver
            <| "//select[@class = 'ui-paginator-rpp-options']"

            Thread.Sleep(3000)

            this.Clicker driver
            <| "//select[@class = 'ui-paginator-rpp-options']/option[5]"
        with
            | ex -> Logging.Log.logger ex
    ...
    member private this.ParserTendersList (_: ChromeDriver) (t: KgRec) =
        try
            let T =
                TenderKg(set, t, 208, "Министерство финансов Кыргызской Республики", "http://zakupki.gov.kg")
    ...
    member __.GetNextpage(driver: ChromeDriver) =
        ...
        try
            jse.ExecuteScript("document.querySelector('a[aria-label=\"Next Page\"]').click()", "")
            |> ignore
    ...
    member private this.ParserTenders(i: IWebElement) =
        ...
        let result =
            builder {
                ...
                let! purNum =
                    i.findElementWithoutException (".//td[1]", sprintf "purNum not found, inner text - %s" i.Text)

                let purNum = purNum.Replace("№", "").Trim()
                let! orgName = i.findElementWithoutException (".//td[2]", sprintf "orgName not found %s" i.Text)

                let orgName =
                    orgName
                        .Replace("НАИМЕНОВАНИЕ ОРГАНИЗАЦИИ", "")
                        .Trim()

                let! purName =
                    i.findElementWithoutException (
                        ".//td[4]/span[@class='nameTender']",
                        sprintf "purName not found %s" i.Text
                    )

                let! pwName = i.findElementWithoutException (".//td[6]", sprintf "purName not found %s" i.Text)

                let pwName =
                    pwName.Replace("МЕТОД ЗАКУПОК", "").Trim()

                let nmck = Tools.InlineFEWE i ".//td[7]"

                let nmck =
                    nmck.Replace("ПЛАНИРУЕМАЯ СУММА", "").Trim()

                let nmck = nmck.GetNmck()
                let! pubDateT = i.findElementWithoutException (".//td[8]", sprintf "pubDateT not found %s" i.Text)

                let pubDateT =
                    pubDateT.Replace("ДАТА ОПУБЛИКОВАНИЯ", "").Trim()
                ...
                let endDateT =
                    endDateT
                        .Replace("СРОК ПОДАЧИ КОНКУРСНЫХ ЗАЯВОК", "")
                        .Trim()
