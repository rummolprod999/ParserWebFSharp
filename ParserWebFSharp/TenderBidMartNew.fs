...
type TenderBidMartNew
    ...
    override this.Parsing() =
        ...
        let res =
            builder {
                ...
                wait.Until(fun dr -> dr.FindElement(By.XPath("//h2")).Displayed)
                |> ignore
                ...
                let! bidDateT =
                    body.findElementWithoutExceptionOptional (
                        "//th[. = 'Дата принятия решения']/following-sibling::td",
                        sprintf <| "bidDateT not found %s" <| tn.Href
                    )

                let mutable biddingDate = DateTime.MinValue

                if bidDateT <> "" then
                    biddingDate <- DateTime.Parse(bidDateT, CultureInfo.CreateSpecificCulture("ru-RU"))
                ...
                let! pwName =
                    body.findElementWithoutExceptionOptional (
                        "//th[. = 'Тип закупки']/following-sibling::td",
                        sprintf <| "pwName not found %s" <| tn.Href
                    )

                let mutable pwName = pwName

                if pwName.Length > 0 && pwName.IndexOf("#") > 0 then
                    pwName <- pwName.Substring(0, pwName.IndexOf("#"))
                ...
                let! delivTerm1 =
                    body.findElementWithoutExceptionOptional (
                        "//th[. = 'Дата поставки']/following-sibling::td",
                        sprintf <| "delivPlace not found %s" <| tn.Href
                    )

                let! delivTerm2 =
                    body.findElementWithoutExceptionOptional (
                        "//th[. = 'Условия поставки']/following-sibling::td",
                        sprintf <| "delivPlace not found %s" <| tn.Href
                    )

                let! delivTerm3 =
                    body.findElementWithoutExceptionOptional (
                        "//th[. = 'Отсрочка платежа']/following-sibling::td",
                        sprintf <| "delivPlace not found %s" <| tn.Href
                    )
                ...
                if tn.DelivPlace <> "" || delivTerm <> "" then
                    ...
                    cmd16.Parameters.AddWithValue("@delivery_place", tn.DelivPlace)
                    |> ignore
                ...
                let purObjects =
                    body.findElementsWithoutException ("//app-tender-lot-items//app-accordion-panel/div")
    ...
    member private this.GetPurObjs
        ...
        for p in pos do
            let name =
                p.findElementWithoutException ("./span[2]")

            let QuantT =
                p.findElementWithoutException ("./span[4]")
            ...
            let okei =
                p.findElementWithoutException ("./span[3]")

            let PriceT =
                p.findElementWithoutException ("./td[4]")

            let mutable Price =
                p.findElementWithoutException ("./span[5]/b")
