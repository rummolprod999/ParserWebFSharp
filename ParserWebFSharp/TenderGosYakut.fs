...
type TenderGosYakut(stn : Settings.T, tn : GosYakutRec, typeFz : int, etpName : string, etpUrl : string) =
    ...
    override this.Parsing() =
        ...
        if reader.HasRows then reader.Close()
            ...
            match updated with
            | true -> incr TenderGosYakut.tenderUpCount
            | false -> incr TenderGosYakut.tenderCount
            ...
            let Okpd2 =
                match doc.QuerySelector("p:contains('ОКПД 2:')") with
                | null -> ""
                | ur -> ur.TextContent.Replace("ОКПД 2:", "").Trim()
            
            let nameT =
                match doc.QuerySelector("p:contains('Краткое описание:')") with
                | null -> ""
                | ur -> ur.TextContent.Replace("Краткое описание:", "").Trim()
            
            let mutable name = tn.PurName
            if tn.PurName <> nameT then name <- (sprintf "%s %s" tn.PurName nameT).Trim()
            let insertLotitem =
                sprintf 
                    "INSERT INTO %spurchase_object SET id_lot = @id_lot, id_customer = @id_customer, name = @name, sum = @sum, okpd2_code = @okpd2_code" 
                    stn.Prefix
            ...
            let documents = doc.QuerySelectorAll("div:contains('Приложенные файлы') + ul li a")
    ...
    member private this.ParsingDocs (con : MySqlConnection) (idTender : int) (elem : IElement) =
        let docName = this.GetDefaultFromNull <| elem
        match docName with
        | "" -> ()
        | x -> 
            let hrefT = elem.GetAttribute("href")
            let href = sprintf "http://market.goszakazyakutia.ru%s" hrefT
            let desc = ""
            let addAttach =
                sprintf 
