        ...
        if reader.HasRows then
            ...
            let nameT =
                ...
                | ur ->
                    ur
                        .TextContent
                        .Replace("Краткое описание:", "")
                        .Trim()

            let mutable name = tn.PurName

            if tn.PurName <> nameT then
                name <- (sprintf "%s %s" tn.PurName nameT).Trim()
            ...
            let documents =
                doc.QuerySelectorAll("div:contains('Приложенные файлы') + ul li a")
