...
type TenderDomRu(stn: Settings.T, tn: DomRuRec, typeFz: int, etpName: string, etpUrl: string) =
    ...
    override this.Parsing() =
        ...
        let res =
            builder {
                ...
                let! purNum =
                    nav.GsnDocWithError
                        "//td[@class = 'oit-h' and contains(., 'Номер&nbsp;закупки')]/following-sibling::td"
                    <| sprintf "purNum is empty on url %s" tn.Href

                if purNum = "" then
                    return! Err(sprintf "purNum is empty %s" tn.Href)
                ...
                let orgName =
                    nav.Gsn "//td[@class = 'oit-h' and contains(., 'Наименование заказчика')]/following-sibling::td"
                ...
                let status =
                    nav.Gsn "//td[@class = 'oit-h' and contains(., 'Состояние закупки')]/following-sibling::td"
    ...
    member private this.GetLots(con: MySqlConnection, idTender: int, doc: HtmlDocument, idCustomer) =
        let lots =
            doc
                .DocumentNode
                .SelectNodes("//h2[. = 'Лоты']/following-sibling::div[1]/table/tr")
                .Skip(1)
    ...
    member private this.GetAttachments(con: MySqlConnection, idTender: int, doc: HtmlDocument) =
        let documents =
            doc.DocumentNode.SelectNodes("//h2[. = 'Сообщения']/following-sibling::div/table/tr")
