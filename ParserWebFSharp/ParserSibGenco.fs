...
type ParserSibGenco(stn: Settings.T) =
    ...
    member private __.ParsingPage(url: string) =
        ...
        | s ->
            ...
            let tens = nav.CurrentDocument.DocumentNode.SelectNodesOrEmpty("//table[@class = 'tender-table']//tbody/tr").ToList()
            tens.Reverse()
            for t in tens do
                    try
    ...
    member private __.ParsingTender (t: HtmlNode) (_: string) =
        ...
        let res = builder {
            let! purName = t.GsnDocWithError "./td[3]/a" <| sprintf "purName not found %s %s " url (t.InnerText)
            let! hrefT = t.GsnAtrDocWithError "./td[3]/a" <| "href" <| sprintf "hrefT not found %s %s " url (t.InnerText)
            let href = sprintf "https://sibgenco.ru%s" hrefT
            let! purNum = t.GsnDocWithError "./td[2]" <| sprintf "purNum not found %s %s " url (t.InnerText)
            ...
            let tend = {  Href = href
                          ...
                          DateEnd = dateEnd
                          DatePub = datePub}          
            let T = TenderSibGenco(set, tend, 289, "ООО «Сибирская генерирующая компания»", "https://sibgenco.ru/")
            T.Parsing()
            return ""
            }
        match res with
