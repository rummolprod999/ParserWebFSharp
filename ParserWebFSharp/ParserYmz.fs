...
type ParserYmz(stn: Settings.T) =
    ...
    let urls =
        [| "https://www.ymzmotor.ru/about/tenders/" |]
    ...
    member private __.ParsingTender (t: IElement) (url: string) =
        ...
        let res =
            builder {
                let! purName =
                    t.GsnDocWithError "p.tender-item__title a"
                    <| sprintf "purName not found %s %s " url (t.TextContent)

                let! href =
                    t.GsnAtrDocWithError "p.tender-item__title a" "href"
                    <| sprintf "href not found %s %s " url (t.TextContent)

                let href =
                    sprintf "https://www.ymzmotor.ru%s" href

                let purNum = createMD5 (purName)
                let datePub = DateTime.Now

                let! dateEndT =
                    t.GsnDocWithError "div.tender-item__descr p"
                    <| sprintf "dateEndT not found %s %s " url (t.TextContent)

                let dateEndT =
                    dateEndT
                        .Replace("Срок подачи заявки до", "")
                        .Trim()
                ...
                let T =
                    TenderYmz(set, tend, 253, "«ЯМЗ»", "https://www.ymzmotor.ru/")
