...
type ParserPetr(stn: Settings.T) =
    ...
    member private this.ParserSelen(driver: ChromeDriver) =
        ...
        wait.Until
            (fun dr ->
            dr.FindElement(By.XPath("//div[contains(@class, 'panel-default') and contains(@class, 'panel-norad')]")).Displayed)
        |> ignore
    ...
    member private this.ParserTendersList (driver: ChromeDriver) (t: EshopRzdRec) =
        try
            let T = TenderPetr (set, t, 219, "Электронный магазин Петрозаводского городского округа", "https://eshop-ptz.ru/", driver)
            T.Parsing()
    ...
    member private this.GetNextPage (driver: ChromeDriver) (wait: WebDriverWait) =
        for i in 2..5 do
            try
                ...
                Thread.Sleep(5000)
                wait.Until (fun dr -> (dr.FindElement (By.XPath("//div[contains(@class, 'panel-default') and contains(@class, 'panel-norad')]"))).Displayed) |> ignore
                this.ParserListTenders driver
            with ex -> Logging.Log.logger (ex)
        ()
        ...
        let result =
            builder {
                let! purName = t.findElementWithoutException
                                   (".//a[contains(@class,'a-link')]/span[1]",
                                    "purName not found")
                let purName = purName.Trim('/').Trim()
                let hrefT = t.FindElement(By.XPath(".//a[contains(@class,'a-link')]"))
                let href = hrefT.GetAttribute("href")
                let! purNumT = t.findElementWithoutException
                                   (".//a[contains(@class,'a-link')]/span[2]",
                                    "purNumT not found")
                let purNum = purNumT.Replace("/", "").Replace("№", "").RegexDeleteWhitespace()
                let statusT = t.FindElement(By.XPath(".//span[contains(@class,'glyphicon-calendar')]"))
                let status = statusT.GetAttribute("title")
                let! priceT = t.findElementWithoutExceptionOptional (".//b[contains(., '₽')]", "")
                let priceTT = priceT.Replace("&nbsp;", "").Replace(",", ".").Replace("₽", "").RegexDeleteWhitespace()
                let! price = priceTT.Get1Optional(@"^([\d\.]+)")
                let currency = "₽"
                let! nameCusT = t.findElementWithoutExceptionOptional (".//div[contains(., 'Заказчик:') and @class = 'ng-binding']", "")
