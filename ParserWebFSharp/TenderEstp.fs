...
type TenderEstp(stn: Settings.T, tn: EstpRec, typeFz: int, etpName: string, etpUrl: string) =
    ...
    override this.Parsing() =
        ...
        let res =
            builder {
                ...
                | _ ->
                    idPlacingWay
                    := this.GetPlacingWay con tn.PlacingWay stn
                ...
                let delivTerm =
                    InlineHtmlNavigator nav "//td[. = 'Описание']/following-sibling::td"
    ...
    member private this.GetLots(con: MySqlConnection, idTender: int, idCustomer, delivTerm: String) =
        ...
        cmd12.Parameters.AddWithValue("@max_price", tn.Price)
        |> ignore

        cmd12.Parameters.AddWithValue("@currency", tn.Currency)
        |> ignore

        cmd12.Parameters.AddWithValue("@lot_name", tn.PurName)
        |> ignore
        ...
        if specUrl <> "" then
            let Page =
                Download.DownloadString <| specUrl
            ...
            if lots <> null then
                for l in lots do
                    ...
                    let insertLotitem =
                        sprintf
                            "INSERT INTO %spurchase_object SET id_lot = @id_lot, id_customer = @id_customer, name = @name, sum = @sum, price = @price, quantity_value = @quantity_value, customer_quantity_value = @customer_quantity_value, okei = @okei, okpd2_code = @okpd2_code, okpd_name = @okpd_name"
                            stn.Prefix
