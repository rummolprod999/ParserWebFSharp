...
type TenderEstp(stn: Settings.T, tn: EstpRec, typeFz: int, etpName: string, etpUrl: string) =
    ...
    override this.Parsing() =
        ...
        let res =
                   builder {
                        ...
                        match tn.PlacingWay with
                        | "" -> ()
                        | _ -> idPlacingWay := this.GetPlacingWay con tn.PlacingWay stn
                        let idTender = ref 0
                        let numVersion = 1
                        ...
                        match updated with
                        | true -> incr TenderEstp.tenderUpCount
                        | false -> incr TenderEstp.tenderCount
                        let idCustomer = ref 0
                        let cusName = tn.OrgName
                        if cusName <> "" then
                            let selectCustomer =
                        ...
                        let delivTerm = InlineHtmlNavigator nav "//td[. = 'Описание']/following-sibling::td"
                        this.GetLots(con, !idTender, !idCustomer, delivTerm)
    ...
    member private this.GetLots(con: MySqlConnection, idTender: int, idCustomer, delivTerm: String) =
        let lotNum = ref 1
        let idLot = ref 0
        ...
        cmd12.Parameters.AddWithValue("@lot_name", tn.PurName) |> ignore
        cmd12.ExecuteNonQuery() |> ignore
        idLot := int cmd12.LastInsertedId
        let specUrl = sprintf "%slots/" tn.Href
        if specUrl <> "" then
            let Page = Download.DownloadString <| specUrl
            if Page = "" || Page = null then failwith <| sprintf "cannot download specification %s" tn.Href
            let htmlDoc = HtmlDocument()
            htmlDoc.LoadHtml(Page)
            let nav = (htmlDoc.CreateNavigator()) :?> HtmlNodeNavigator
            let lots = htmlDoc.DocumentNode.SelectNodes("//table[contains(., 'Позиции лота')]//tr[position() > 2]")
            if lots <> null then
                for l in lots do
                    ...
                    let price = ""
                    let sum = tn.Price
                    let insertLotitem = sprintf "INSERT INTO %spurchase_object SET id_lot = @id_lot, id_customer = @id_customer, name = @name, sum = @sum, price = @price, quantity_value = @quantity_value, customer_quantity_value = @customer_quantity_value, okei = @okei, okpd2_code = @okpd2_code, okpd_name = @okpd_name" stn.Prefix
            ...
            let delivPlace = InlineHtmlNavigator nav "//td[. = 'Место поставки (адрес)']/following-sibling::td"
    ...
    member private this.GetAttachments(con: MySqlConnection, idTender: int) =
        ...
        if specUrl <> "" then
            ...
            let htmlDoc = HtmlDocument()
            htmlDoc.LoadHtml(Page)
            let docs = htmlDoc.DocumentNode.SelectNodes("//a[contains(@href, '/file/download/')]")
            if docs <> null then
                for d in docs do
                    let docName = d.Gsn("./parent::td/preceding-sibling::td[3]").RegexCutWhitespace()
