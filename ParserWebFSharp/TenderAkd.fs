...
type TenderAkd(stn: Settings.T, urlT: string, purNum: string) =
    inherit Tender
        (
            "Электронная торговая площадка для проведения торгов - Аукционный Конкурсный Дом",
    ...
    override this.Parsing() =
        ...
        match Page with
        | null
        | "" -> Logging.Log.logger ("Dont get page", urlT)
        | s -> this.ParserPage(s)
    ...
    member private this.ParsingDocs (con: MySqlConnection) (idTender: int) (elem: IElement) =
        ...
        | x ->
            let hrefT =
                elem
                    .QuerySelector("td.name a")
                    .GetAttribute("href")

            let href =
                sprintf "http://www.a-k-d.ru%s" hrefT

            let desc =
                this.GetDefaultFromNull
                <| elem.QuerySelector("td:last-child")
            ...
            cmd5.Parameters.AddWithValue("@description", desc)
            |> ignore
    ...
    member private this.ParserPage(p: string) =
        ...
        let pubDateT =
            doc.QuerySelector("th:contains('Публикация электронной процедуры') + td > span")
        ...
        let datePub =
            ...
            | None ->
                ...
                | None ->
                    raise
                    <| Exception(sprintf "cannot parse datePub %s, %s" pubDateS urlT)

        let endDateT =
            doc.QuerySelector("th:contains('Окончание приема заявок') + td > span")
        ...
        let scoringDateT =
            doc.QuerySelector("th:contains('Дата окончания') + td > span")
        ...
        let biddingDateT =
            doc.QuerySelector("th:contains('Начало') + td > span")
        ...
        let NoticeVersion =
            this.GetDefaultFromNull
            <| doc.QuerySelector("th:contains('Статус') + td > span")
        ...
        if reader.HasRows then
            ...
            let OrgName =
                this.GetDefaultFromNull
                <| doc.QuerySelector("th:contains('Организатор торгов') + td")
            ...
            | _ ->
                ...
                | false ->
                    ...
                    let contactPerson =
                        this.GetDefaultFromNull
                        <| doc.QuerySelector("th:contains('Контактное лицо') + td")

                    let postAddress =
                        this.GetDefaultFromNull
                        <| doc.QuerySelector("th:contains('Почтовый адрес') + td")

                    let factAddress =
                        this.GetDefaultFromNull
                        <| doc.QuerySelector("th:contains('Фактический адрес') + td")
            ...
            let PlacingWayName =
                this.GetDefaultFromNull
                <| doc.QuerySelector("th:contains('Наименование способа размещения') + td")
            ...
            let documents =
                doc.QuerySelectorAll("tbody.zen-direct-upload-container > tr")
            ...
            let maxPriceT =
                this.GetDefaultFromNull
                <| doc.QuerySelector("th:contains('Начальная цена') + td > span")
            ...
            let financeSource =
                this.GetDefaultFromNull
                <| doc.QuerySelector("th:contains('Источник финансирования') + td")
            ...
            cmd12.Parameters.AddWithValue("@finance_source", financeSource)
            |> ignore
            ...
            let CustomerName =
                this.GetDefaultFromNull
                <| doc.QuerySelector("th:contains('Заказчик') + td")
            ...
            cmd19.Parameters.AddWithValue("@sum", maxPrice)
            |> ignore
            ...
            let delivTerm =
                this.GetDefaultFromNull
                <| doc.QuerySelector("th:contains('Сроки исполнения') + td")

            let delivTerm1 =
                this.GetDefaultFromNull
                <| doc.QuerySelector("th:contains('Сроки и условия оплаты') + td")

            match (delivTerm, delivTerm1) with
            | (_, _) & ("", "") -> ()
            | (x, y) ->
                let deliv =
                    sprintf "Сроки исполнения: %s \n Сроки и условия оплаты: %s" x y
                ...
                cmd16.Parameters.AddWithValue("@delivery_term", deliv)
                |> ignore
