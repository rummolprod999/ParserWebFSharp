...
type TenderTj(stn: Settings.T, tn: TjRec, typeFz: int, etpName: string, etpUrl: string) =
    ...
    override this.Parsing() =
        ...
        let res =
                   builder {
                        ...
                        let IdOrg = ref 0
                        let orgName = nav.Gsn "//th[contains(., 'Организатор:')]/following-sibling::td"
                        ...
                        if orgName <> "" then
                            ...
                            | false ->
                                ...
                                let contactPerson = InlineHtmlNavigator nav "//th[contains(., 'Контактная информация:')]/following-sibling::td"
                                let postAddress = ""
                                let factAddress = ""
                                let phone = ""
                                let inn = ""
    ...
    member private this.GetLots(con: MySqlConnection, idTender: int, doc: HtmlDocument) =
        ...
        let lotName = nav.Gsn "//th[contains(., 'Наименование лота:')]/following-sibling::td/strong"
        let cusName = nav.Gsn "//th[contains(., 'Закупающая организация:')]/following-sibling::td/strong"
