...
type ParserBhm(stn: Settings.T) =
    ...
    let urls =
        [| "https://bhm.ru/tender/"
           "https://bhm.ru/tender/?PAGEN_1=2"
           "https://bhm.ru/tender/?PAGEN_1=3" |]
    ...
    member private __.ParsingPage(url: string) =
        ...
        | s ->
            ...
            let tens =
                documents
                    .QuerySelectorAll("div.tenders__item")
                    .ToList()
    ...
    member private __.ParsingTender (t: IElement) (url: string) =
        ...
        let res =
            builder {
                let! purName =
                    t.GsnDocWithError "div.tenders__item-text"
                ...
                let! datePubT =
                    t.GsnDocWithError "div.tenders__item-topbar > div.tenders__item-date"
                ...
                let! dateEndT =
                    t.GsnDocWithError "div.tenders__item-deadline"
                    <| sprintf "dateEndT not found %s %s " url (t.TextContent)

                let dateEnd =
                    if dateEndT.Contains("рабочих дня с даты размещения") then
                        let dateEndP =
                            dateEndT.Get1FromRegexpOrDefaul("(\d+) рабочих дня с даты размещения")

                        datePub.AddDays(float (Int32.Parse(dateEndP)))
                    else
                        let dateEndP =
                            dateEndT.Get1FromRegexpOrDefaul("(\d{2}\.\d{2}\.\d{4})")
                ...
                let tens =
                    t
                        .QuerySelectorAll("div.tenders__item-text table tbody tr")
                        .ToList()
                        .Skip(1)
                ...
                let attachments =
                    t
                        .QuerySelectorAll("div.tenders__item-text a")
                        .ToList()

                let listAtt =
                    __.CreateAttachments(attachments)
                ...
                let T =
                    TenderBhm(set, ten, 275, "АО «Борхиммаш»", "https://bhm.ru/")
    ...
    member private __.CreateProducts(elements) =
        ...
        for p in elements do
            ...
            let pNameT =
                p.QuerySelector("td:nth-child(1)")

            if pNameT <> null then
                pName <- pNameT.TextContent.Trim()

            let okeiT =
                p.QuerySelector("td:nth-child(3)")

            if okeiT <> null then
                okei <- okeiT.TextContent.Trim()

            let quantT =
                p.QuerySelector("td:nth-child(4)")

            if quantT <> null then
                quant <- quantT.TextContent.Trim()
    ...
    member private __.CreateAttachments(elements) =
        ...
        for p in elements do
            ...
            listDoc.Add({ name = attName; url = href })
