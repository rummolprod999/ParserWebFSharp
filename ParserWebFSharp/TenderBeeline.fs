...
type TenderBeeline(stn : Settings.T, tn : BeelineRec, typeFz : int, etpName : string, etpUrl : string) =
    ...
    override this.Parsing() =
        ...
        let selectTend =
            sprintf
                "SELECT id_tender FROM %stender WHERE purchase_number = @purchase_number AND type_fz = @type_fz AND doc_publish_date = @doc_publish_date"
                stn.Prefix
        ...
        if reader.HasRows then reader.Close()
                ...
                | false ->
                    ...
                    let addOrganizer =
                        sprintf
                            ...
                            stn.Prefix
                    let contactPerson = ""
                    let postAddress = "Российская Федерация, 127083, г. Москва, ул. Восьмого Марта, дом 10, строение 14"
                    let factAddress = ""
                    let phone = ""
                    let inn = "7713076301"
            ...
            match updated with
            | true -> incr TenderBeeline.tenderUpCount
            | false -> incr TenderBeeline.tenderCount
