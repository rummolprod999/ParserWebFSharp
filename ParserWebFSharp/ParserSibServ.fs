...
type ParserSibServ(stn: Settings.T) =
    ...
    let url =
        "https://tp.sibserv.com/tenders.php"
    ...
    member private this.ParserSelen(driver: ChromeDriver) =
        ...
        wait.Until (fun dr ->
            dr
                .FindElement(
                    By.XPath("//div[@class = 'defc']/div[@class = 'line']/following-sibling::p[1]")
                )
                .Displayed)
        |> ignore
        ...
        let tenders =
            driver.FindElementsByXPath("//div[@class = 'defc']/div[@class = 'line']")
    ...
    member private this.ParserTenders (_: ChromeDriver) (t: IWebElement) =
        ...
        let purName =
            ...
            | "" ->
                raise
                <| NullReferenceException(sprintf "purNume not found in %s" url)
            | x -> x
        ...
        let dateT =
            ...
            <| t.FindElement(By.XPath("./following-sibling::p[3]"))

        let datePubT =
            match dateT
                .RegexCutWhitespace()
                .Get1FromRegexp @"Дата публикации лота:\s(\d{2}\.\d{2}\.\d{4}\s\d{2}:\d{2})"
                with
            | Some x -> x.Trim()
            | None ->
                raise
                <| NullReferenceException(sprintf "datePubT not found in %s %s" url dateT)
        ...
        let datePub =
            ...
            | None ->
                raise
                <| Exception(sprintf "cannot parse datePubT %s, %s" datePubT url)

        let dateEndT =
            match dateT
                .RegexCutWhitespace()
                .Get1FromRegexp @"Дата окончания приема заявок:\s(\d{2}\.\d{2}\.\d{4}\s\d{2}:\d{2})"
                with
            | Some x -> x.Trim()
            | None ->
                raise
                <| NullReferenceException(sprintf "dateEndT not found in %s %s" url dateT)
        ...
        try
            let T =
                TenderSibServ(set, ten, 99, "АО «Сибирская Сервисная Компания»", "https://tp.sibserv.com")

            T.Parsing()
        with
            | ex -> Logging.Log.logger (ex, href)
