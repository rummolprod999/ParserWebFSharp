...
type TenderVend(stn : Settings.T, tn : VendRec, typeFz : int, etpName : string, etpUrl : string) =
    ...
    override this.Parsing() =
        ...
        if reader.HasRows then reader.Close()
            ...
            let Printform = href
            let IdOrg = ref 0
            let OrgName = HttpUtility.HtmlDecode(nav.Gsn("//th[contains(., 'Организация:')]/following-sibling::td/a"))
            if OrgName <> "" then 
                ...
                | false -> 
                    ...
                    ()
            let notice_version = nav.Gsn("//th[label[contains(., 'Статус')]]/following-sibling::td/label")
            ...
            cmd9.Parameters.AddWithValue("@notice_version", notice_version) |> ignore
            ...
            match updated with
            | true -> incr TenderVend.tenderUpCount
            | false -> incr TenderVend.tenderCount
            ...
            let currency = nav.Gsn("//th[label[contains(., 'Валюта')]]/following-sibling::td")
            let nmck =
                nav.Gsn("//th[label[contains(., 'Максимальная цена товаров, работ, услуг')]]/following-sibling::td")
                   .RegexDeleteWhitespace().Replace(",", ".")
            let idLot = ref 0
            let insertLot =
                sprintf 
                        ...
                        if docName <> "" && docUrl <> "" then 
                            let d =
                                { name = docName.RegexCutWhitespace()
                                  url = sprintf "http://vendorportal.ru%s" docUrl }
                            docList.Add(d)
            ...
            for d in docList do
                ...
                cmd5.ExecuteNonQuery() |> ignore
            let purObjects =
                htmlDoc.DocumentNode.SelectNodes("//div[@id = 'offerinvitationlots']/table//tr[position() > 1]")
            match purObjects with
            | null -> 
                let insertLotitem =
                    sprintf 
            ...
            | _ -> 
                for po in purObjects do
                    let pname = HttpUtility.HtmlDecode(po.Gsn("./td[1]"))
                    let okei = po.Gsn("./td[3]")
                    let quant = po.Gsn("./td[4]").Replace(",", ".")
                    let insertLotitem =
                        sprintf 
            ...
            let delivTerms =
                htmlDoc.DocumentNode.SelectNodes
                    ("//div[@id = 'offerinvitationdeliveries']/table//tr[position() > 1]")
            match delivTerms with
            | null -> ()
            | _ -> 
                for terms in delivTerms do
                    let delivTerm1 = HttpUtility.HtmlDecode(terms.Gsn("./td[2]"))
                    let delivTerm2 = HttpUtility.HtmlDecode(terms.Gsn("./td[4]"))
                    let delivTerm = (sprintf "%s %s" delivTerm1 delivTerm2).RegexCutWhitespace().Trim()
                    let delivPlace = HttpUtility.HtmlDecode(terms.Gsn("./td[3]"))
