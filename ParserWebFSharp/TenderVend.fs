...
type TenderVend(stn: Settings.T, tn: VendRec, typeFz: int, etpName: string, etpUrl: string) =
    ...
    override this.Parsing() =
        ...
        if reader.HasRows then
            ...
            let OrgName =
                HttpUtility.HtmlDecode(nav.Gsn("//th[contains(., 'Организация:')]/following-sibling::td/a"))
            ...
            let notice_version =
                nav.Gsn("//th[label[contains(., 'Статус')]]/following-sibling::td/label")
            ...
            let currency =
                nav.Gsn("//th[label[contains(., 'Валюта')]]/following-sibling::td")

            let nmck =
                nav
                    .Gsn("//th[label[contains(., 'Максимальная цена товаров, работ, услуг')]]/following-sibling::td")
            ...
            let docs =
                htmlDoc.DocumentNode.SelectNodes("//div[contains(@class, 'fileUploadLink')]/a")
            ...
            | _ ->
                for po in purObjects do
                    let pname =
                        HttpUtility.HtmlDecode(po.Gsn("./td[1]"))
            ...
            let delivTerms =
                htmlDoc.DocumentNode.SelectNodes("//div[@id = 'offerinvitationdeliveries']/table//tr[position() > 1]")
            ...
            | _ ->
                for terms in delivTerms do
                    let delivTerm1 =
                        HttpUtility.HtmlDecode(terms.Gsn("./td[2]"))

                    let delivTerm2 =
                        HttpUtility.HtmlDecode(terms.Gsn("./td[4]"))
                    ...
                    let delivPlace =
                        HttpUtility.HtmlDecode(terms.Gsn("./td[3]"))
