...
type ParserEshopRzd(stn: Settings.T) =
    ...
    member private this.ParserTendersList (driver: ChromeDriver) (t: EshopRzdRec) =
        try
            let T =
                TenderEshopRzd(set, t, 113, "Электронный магазин ОАО \"РЖД\"", "https://eshoprzd.ru/", driver)
    ...
    member private this.ParserTenders (_: ChromeDriver) (t: IWebElement) =
        ...
        let result =
            builder {
                ...
                let! datePubT =
                    t.findElementWithoutException (
                        ".//div[span[contains(., 'Дата публикации:')]]",
                        "datePubT not found"
                    )
                ...
                let! dateEndT =
                    t.findElementWithoutException (".//div[span[contains(., 'Подача оферт до')]]", "dateEndT not found")
