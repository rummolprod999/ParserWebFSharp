...
type ParserBarnaulTm(stn: Settings.T) =
    inherit Parser()
    let set = stn
    let url ="http://www.barnaultransmash.ru/tenders"
    ...
    member private __.ParserSelen(driver : ChromeDriver) =
        ...
        wait.Until
            (fun dr -> 
            dr.FindElement(By.XPath("//a[@class = 'js-feed-post-link']")).Displayed) |> ignore
        driver.SwitchTo().DefaultContent() |> ignore
    ...
    member private this.ParserTendersList (driver : ChromeDriver) (t : RosSelRec) =
        try 
            let T = TenderBarnaulTm(set, t, 333, "«Барнаултрансмаш»", "http://www.barnaultransmash.ru/", driver)
            T.Parsing()
