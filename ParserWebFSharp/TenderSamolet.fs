...
type TenderSamolet(stn: Settings.T, tn: SamoletRec, typeFz: int, etpName: string, etpUrl: string, driver: ChromeDriver) =
    ...
    override this.Parsing() =
        ...
        wait.Until (fun dr ->
            (dr.FindElement(
                By.XPath("//div[contains(@class, 'mat-tab-label-content') and contains(., 'Извещение и документация')]")
            ))
                .Displayed)
        |> ignore
        ...
        let dateEndT =
            match dateEndT with
            | x when dateEndT.Contains("Завтра в") ->
                dateEndT.Replace("Завтра в", DateTime.Now.AddDays(1.).ToString("dd.MM.yyyy"))
            | x when dateEndT.Contains("Сегодня в") ->
        ...
        let dateEnd =
            ...
            | None ->
                raise
                <| Exception(sprintf "cannot parse dateEndT %s, %s" dateEndT tn.Href)

        let datePubT =
            driver.findElementWithoutException (
                "//div[contains(., 'Начало подтверждения участия')]/following-sibling::div"
            )

        let datePubT =
            match datePubT with
            | x when datePubT.Contains("Завтра в") ->
                datePubT.Replace("Завтра в", DateTime.Now.AddDays(1.).ToString("dd.MM.yyyy"))
            | x when datePubT.Contains("Сегодня в") ->
                datePubT.Replace("Сегодня в", DateTime.Now.ToString("dd.MM.yyyy"))
            | x when datePubT.Contains("Вчера в") ->
                datePubT.Replace("Вчера в", DateTime.Now.AddDays(-1.).ToString("dd.MM.yyyy"))
            | _ -> datePubT

        let datePubT =
            datePubT.Get1FromRegexpOrDefaul("(\d{2}.\d{2}.\d{4} \d{2}:\d{2})")
        ...
        let datePub =
            ...
            | None ->
                raise
                <| Exception(sprintf "cannot parse datePubT %s, %s" datePubT tn.Href)

        let dateScoringT =
            driver.findElementWithoutException ("//div[. = 'Дата подведения итогов (ТК)']/following-sibling::div")
        ...
        let status =
            driver.findElementWithoutException ("//div[contains(@class,'-status-button-title')]")
        ...
        if reader.HasRows then
            ...
            if OrgName <> "" then
                ...
                | false ->
                    ...
                    let contactPerson =
                        driver.findElementWithoutException (
                            "//div[contains(., 'Контактное лицо')]/following-sibling::div/div"
                        )

                    let postAddress =
                        "МОСКВА Г, ИВАНА ФРАНКО УЛ, ДОМ 8, ЭТАЖ/КОМН. 10/23"

                    let factAddress = ""

                    let phone =
                        driver.findElementWithoutException (
                            "//div[contains(., 'Адрес электронной почты')]/following-sibling::div/div"
                        )
            ...
            let placingWay =
                driver.findElementWithoutException ("//div[contains(., 'Способ закупки')]/following-sibling::div")
            ...
            | _ ->
                idPlacingWay
                := this.GetPlacingWay con placingWay settings
            ...
            let attachments =
                driver.FindElements(By.XPath("//um-files-dropdown/a"))
            ...
            for att in attachments do
                ...
                if urlAtt <> "" && urlName <> "" then
                    ...
                    cmd5.Parameters.AddWithValue("@file_name", urlName)
                    |> ignore

                    cmd5.Parameters.AddWithValue("@url", urlAtt)
                    |> ignore
            ...
            let lots =
                driver.FindElements(By.XPath("//mat-expansion-panel"))

            let lotNum = ref 1

            for l in lots do
                let lotName =
                    l.findElementWithoutException (".//div[. = 'Наименование лота']/following-sibling::div/div/span")
                ...
                cmd12.Parameters.AddWithValue("@currency", "RUB")
                |> ignore
                ...
                let delivlace =
                    driver.findElementWithoutException ("//div[contains(., 'Условия поставки')]/following-sibling::div")

                let delivTerm1 =
                    driver.findElementWithoutException ("//div[contains(., 'Сроки поставки')]/following-sibling::div")

                let delivTerm2 =
                    driver.findElementWithoutException ("//div[contains(., 'Условия оплаты')]/following-sibling::div")

                let delivTerm3 =
                    driver.findElementWithoutException (
                        "//div[contains(., 'Пояснение условий поставки')]/following-sibling::div"
                    )

                let delivTerm =
                    sprintf
                        "Сроки поставки: %s\nУсловия оплаты:%s\nПояснение условий поставки:%s"
                        delivTerm1
                        delivTerm2
                        delivTerm3
                ...
                if delivlace <> "" || delivTerm <> "" then
                    ...
                    cmd16.Parameters.AddWithValue("@delivery_place", delivlace)
                    |> ignore
