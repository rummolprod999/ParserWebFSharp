...
type ParserSmart(stn: Settings.T) =
    ...
    let spage =
        "https://smarttender.biz/komertsiyni-torgy/?p="
    ...
    member private __.ParsingPage(url: string) =
        ...
        | s ->
            ...
            let tens =
                documents.QuerySelectorAll("#tenders > tbody > tr.head")
    ...
    member private __.ParsingTender (t: IElement) (url: string) =
        ...
        let res =
            builder {
                let! href =
                    t.GsnAtrDocWithError "a.linkSubjTrading" "href"
                    <| sprintf "href not found %s %s " url (t.TextContent)

                let! purName =
                    t.GsnDocWithError "a.linkSubjTrading"
                    <| sprintf "purName not found %s %s " url (t.TextContent)

                let! purNum =
                    t.GsnDocWithError "td.col1 > span"
                    <| sprintf "purNum not found %s %s " url (t.TextContent)

                let! orgName =
                    t.GsnDocWithError "td.col3 a.organizer-popover"
                ...
                let! dateEndT =
                    t.GsnDocWithError "td.col6 > div"
                    <| sprintf "dateEndT not found %s %s " url (t.TextContent)

                let! dateEndS = dateEndT.Get1OptionalDoc("(\d{2}\.\d{2}\.\d{4}.+\d{2}:\d{2})")

                let dateEndS =
                    HttpUtility.HtmlDecode(dateEndS)

                let dateEndS = dateEndS.RegexCutWhitespace()

                let! dateEnd =
                    dateEndS.DateFromStringDoc("dd.MM.yyyy HH:mm", sprintf "dateEndT not found %s %s " href dateEndT)
                ...
                let T =
                    TenderSmart(set, tend, 194, "SmartTender", "https://smarttender.biz/")
