...
type ParserBeeline(stn : Settings.T) as __ =
    ...
    member private __.ParserSelen(driver : ChromeDriver) =
        ...
        wait.Until
            (fun dr ->
            dr.FindElement(By.XPath("//a[@class = 'ProductList_product_ghAg']")).Displayed)
        |> ignore
    ...
    member private __.ParserTendersList(t : BeelineRec) =
        try
            let T = TenderBeeline(set, t, 135, "ПАО «ВымпелКом»", "https://beeline.ru")
            T.Parsing()
        ...
        let result =
            builder {
                ...
                let! purName = i.findElementWithoutException
                                   (".//div[@class = 'ProductList_description_2eFc']", sprintf "purName not found %s" i.Text)
                let purNum = Tools.createMD5 href
                let! datePubT = i.findElementWithoutException
