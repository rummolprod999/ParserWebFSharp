...
type ParserUnipro(stn: Settings.T) =
    ...
    let urls =
        [| "http://unipro.energy/purchase/announcement/?PAGEN_1=" |]
    ...
    member private __.ParsingPage(url: string) =
        ...
        | s ->
            ...
            let tens =
                documents
                    .QuerySelectorAll("#table_procurement tbody tr")
                    .ToList()
    ...
    member private __.ParsingTender (t: IElement) (t1: IElement) (url: string) =
        ...
        let res =
            builder {
                let! purName =
                    t.GsnDocWithError "td.table_title"
                    <| sprintf "purName not found %s %s " url (t.TextContent)

                let! href =
                    t1.GsnAtrDocWithError "a" "href"
                    <| sprintf "href not found %s %s " url (t.TextContent)

                let href =
                    sprintf "http://unipro.energy%s" href
                ...
                let! dates =
                    t.GsnDocWithError "td.table_time"
                    <| sprintf "dates not found %s %s " url (t.TextContent)

                let! datePubT =
                    dates.Get1Doc "^(\d{2}\.\d{2}\.\d{2})"
                    <| sprintf "datePubT not found %s %s " href (dates)

                let datePubT =
                    datePubT.RegexCutWhitespace().Trim()

                let! datePub = datePubT.DateFromStringDoc("dd.MM.yy", sprintf "datePub not found %s %s " href datePubT)

                let mutable dateEndT =
                    dates.Get1FromRegexpOrDefaul "Продлено до (\d{2}\.\d{2}\.\d{2})"

                if dateEndT = "" then
                    dateEndT <- dates.Get1FromRegexpOrDefaul "(\d{2}\.\d{2}\.\d{2} \d{2}:\d{2}:\d{2})"

                let mutable dateEnd =
                    dateEndT.DateFromStringOrMin("dd.MM.yy")
                ...
                let T =
                    TenderUnipro(set, tend, 254, "ПАО «Юнипро»", "http://unipro.energy/")
