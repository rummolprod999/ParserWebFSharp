...
type ParserNeftReg(stn: Settings.T) =
    ...
    let spage =
        "https://etp.neftregion.ru/auctions"
    ...
    member private __.ParsingPage(url: string) =
        ...
        | s ->
            ...
            let tens =
                documents
                    .QuerySelectorAll("table.tableorg tr[id ^='trauction']")
                    .ToList()
    ...
    member private __.ParsingTender (t: IElement) (t1: IElement) (url: string) =
        ...
        let res =
            builder {
                let! href =
                    t1.GsnAtrDocWithError "a[href ^= '/view-auction']" "href"
                    <| sprintf "href not found %s %s " url (t.TextContent)

                let href =
                    sprintf "https://etp.neftregion.ru%s" href

                let! purName =
                    t.GsnDocWithError "td:nth-of-type(2) > div:nth-of-type(1)"
                ...
                let! purNum =
                    t.GsnDocWithError "td:nth-of-type(1) > span:nth-of-type(1)"
                ...
                let T =
                    TenderNeftReg(
                        ...
                        tend,
                        232,
                        "ЭТП мелкооптового рынка нефтепродуктов «НефтьРегион»",
                        "https://etp.neftregion.ru/"
                    )
