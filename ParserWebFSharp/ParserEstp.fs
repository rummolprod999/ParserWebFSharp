...
type ParserEstp(stn: Settings.T) =
    ...
    let url =
        "http://estp.ru/auctions/estp/?PAGEN_2="
        ...
        | s ->
            ...
            let tens =
                nav
                    .CurrentDocument
                    .DocumentNode
                    .SelectNodesOrEmpty("//li[@class = 'estp-tenders-list__i']")
                    .ToList()
    ...
    member private __.ParsingTender (t: HtmlNode) (_: string) =
        ...
        let res =
            builder {
                ...
                let! purNum =
                    t.GsnDocWithError ".//dt[. = 'Код закупки:']/following-sibling::dd/strong"
                    <| sprintf "purNum not found %s %s " url (t.InnerText)

                let! hrefT =
                    t.GsnAtrDocWithError ".//dd[@class = 'estp-tenders__data-definition']/a"
                    <| "href"
                ...
                let! purName =
                    t.GsnDocWithError ".//dd[@class = 'estp-tenders__data-definition']/a"
                    <| sprintf "purName not found %s %s " url (t.InnerText)

                let! region = t.GsnDoc ".//dt[. = 'Регион:']/following-sibling::dd"

                let! datePubT =
                    t.GsnDocWithError ".//dt[. = 'Дата публикации: (МСК)']/following-sibling::dd/time"
                ...
                let! dateEndT =
                    t.GsnDocWithError ".//dt[. = 'Дата окончания приема заявок: (МСК)']/following-sibling::dd/time"
                    <| sprintf "dateEndT not found %s %s " href (t.InnerText)
