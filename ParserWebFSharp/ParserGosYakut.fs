...
type ParserGosYakut(stn : Settings.T) =
    inherit Parser()
    let set = stn
    let url = "http://market.goszakazyakutia.ru/orders/published"
    ...
    override this.Parsing() =
        ...
        | s -> 
            ...
            let tens = documents.QuerySelectorAll("div.table-responsive table.table tbody tr")
            for t in tens do
                try 
    ...
    member private this.ParsingTender (t : IElement) (url : string) =
        ...
        let HrefCus = sprintf "http://market.goszakazyakutia.ru%s" HrefCusT
        ...
        let PwName = PwNameT.Replace(PurNum, "").Trim()
        ...
        let dd = (fun (s : string) -> s.Replace(",", "")) >> (fun (s : string) -> Tools.GetDateFromStringMonth(s))
        ...
        let EndDateT =
            ...
            | null -> raise <| System.NullReferenceException(sprintf "EndDateT not found in %s" url)
            | ur -> ur.TextContent.Trim().RegexCutWhitespace()
        ...
        try 
            let T = TenderGosYakut(set, ten, 76, "«WEB-Маркет закупок» Республики Саха (Якутия)", "http://market.goszakazyakutia.ru")
            T.Parsing()
