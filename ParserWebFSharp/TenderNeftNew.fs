    ...
    override this.Parsing() =
        ...
        let res =
                   builder {
                        ...
                        let url = sprintf "https://apizakupki.nefteavtomatika.ru/api/purchases/%s" tn.PurNum
                        let href = sprintf "https://zakupki.nefteavtomatika.ru/purchases/%s/show" tn.PurNum
                        ...
                        if tn.OrgName <> "" then
                            ...
                            | false ->
                                ...
                                let postAddress = ""
                                let factAddress = ""
                                let phone = GetStringFromJtoken t "purchase.author.phone"
                                let email = GetStringFromJtoken t "purchase.author.email"
                                let inn = ""
                        ...
                        let delivTerm = GetStringFromJtoken t "purchase.delivery.time_required"
                        let delivPlace = GetStringFromJtoken t "purchase.delivery.point"
                        let delivTerm1 = GetStringFromJtoken t "purchase.payment_terms"
                        let delivTerm = sprintf "Требуемые сроки поставки: %s\nОбщие условия оплаты: %s" delivTerm delivTerm1
