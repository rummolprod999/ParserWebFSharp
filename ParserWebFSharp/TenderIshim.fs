...
type TenderIshim(stn: Settings.T, tn: OsnovaRec, typeFz: int, etpName: string, etpUrl: string, driver : ChromeDriver) =
    ...
    override this.Parsing() =
        ...
        let res =
                   builder {
                        ...
                        wait.Until (fun dr -> 
            dr.FindElement(By.XPath("//b[. = 'Дата начала:']")).Displayed) |> ignore
                        driver.SwitchTo().DefaultContent() |> ignore
                        ...
                        match updated with
                        | true -> incr TenderIshim.tenderUpCount
                        | false -> incr TenderIshim.tenderCount
                        let attachments = body.findElementsWithoutException("//a[starts-with(@href, '/bitrix/components')]")
                        this.GetAttachments(con, !idTender, attachments)
                        let!  cusName = body.findElementWithoutExceptionOptional("//b[contains(.,'Предприятие-инициатор:')]/parent::p", sprintf <|"cusName not found %s" <| tn.Href)
                        let cusName = cusName.Replace("Предприятие-инициатор:", "").Trim()
                        ...
                        let idLot = ref 0
                        let lotNum = ref 1
                        let purObjects = body.findElementsWithoutException("//table[@class = 't_lot_table']/tbody/tr[position() > 1]")
                        if purObjects.Count = 0 then
                            let currency = "RUB"
                            let finSource = ""
                            ...
                            let! delivTerm1 = body.findElementWithoutExceptionOptional("//b[contains(.,'Срок поставки:')]/parent::p", sprintf <|"delivPlace not found %s" <| tn.Href)
                            let! delivTerm2 = body.findElementWithoutExceptionOptional("//b[contains(.,'Условия оплаты:')]/parent::p", sprintf <|"delivPlace not found %s" <| tn.Href)
                            let! delivPlace = body.findElementWithoutExceptionOptional("//b[contains(.,'Условия поставки:')]/parent::p", sprintf <|"delivPlace not found %s" <| tn.Href)
                            ...
                            this.GetPurObjs(con, !idTender, purObjects, lotNum, !idCustomer, !idTender, body)
     ...
     member private this.GetPurObjs(con: MySqlConnection, _: int, pos: ReadOnlyCollection<IWebElement>, lotNum: int ref, idCustomer: int, idTender, body) =
        for p in pos do
            let currency = "RUB"
            let finSource = ""
            let idLot = ref 0
            let nmck = p.findElementWithoutException("./td[3]").Trim()
            let insertLot = sprintf "INSERT INTO %slot SET id_tender = @id_tender, lot_number = @lot_number, max_price = @max_price, currency = @currency, finance_source = @finance_source" stn.Prefix
            let cmd12 = new MySqlCommand(insertLot, con)
            cmd12.Parameters.AddWithValue("@id_tender", idTender) |> ignore
            ...
            idLot := int cmd12.LastInsertedId
            incr lotNum
            let delivTerm1 = body.findElementWithoutException("//b[contains(.,'Срок поставки:')]/parent::p")
            let delivTerm2 = body.findElementWithoutException("//b[contains(.,'Условия оплаты:')]/parent::p")
            let delivPlace = body.findElementWithoutException("//b[contains(.,'Условия поставки:')]/parent::p")
            let delivTerm = sprintf "%s\n%s" delivTerm1 delivTerm2
            if delivPlace <> ""  || delivTerm <> "" then
                let insertCustomerRequirement =
                    sprintf
