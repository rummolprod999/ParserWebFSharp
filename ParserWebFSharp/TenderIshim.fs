...
type TenderIshim(stn: Settings.T, tn: OsnovaRec, typeFz: int, etpName: string, etpUrl: string, driver: ChromeDriver) =
    ...
    override this.Parsing() =
        ...
        let res =
            builder {
                ...
                let attachments =
                    body.findElementsWithoutException ("//a[starts-with(@href, '/bitrix/components')]")

                this.GetAttachments(con, !idTender, attachments)

                let! cusName =
                    body.findElementWithoutExceptionOptional (
                        "//b[contains(.,'Предприятие-инициатор:')]/parent::p",
                        sprintf <| "cusName not found %s" <| tn.Href
                    )

                let cusName =
                    cusName
                        .Replace("Предприятие-инициатор:", "")
                        .Trim()
                ...
                let purObjects =
                    body.findElementsWithoutException ("//table[@class = 't_lot_table']/tbody/tr[position() > 1]")
                ...
                if purObjects.Count = 0 then
                    ...
                    let! delivTerm1 =
                        body.findElementWithoutExceptionOptional (
                            "//b[contains(.,'Срок поставки:')]/parent::p",
                            sprintf <| "delivPlace not found %s" <| tn.Href
                        )

                    let! delivTerm2 =
                        body.findElementWithoutExceptionOptional (
                            "//b[contains(.,'Условия оплаты:')]/parent::p",
                            sprintf <| "delivPlace not found %s" <| tn.Href
                        )

                    let! delivPlace =
                        body.findElementWithoutExceptionOptional (
                            "//b[contains(.,'Условия поставки:')]/parent::p",
                            sprintf <| "delivPlace not found %s" <| tn.Href
                        )
    ...
    member private this.GetPurObjs
        ...
        for p in pos do
            ...
            let nmck =
                p.findElementWithoutException("./td[3]").Trim()
            ...
            let delivTerm1 =
                body.findElementWithoutException ("//b[contains(.,'Срок поставки:')]/parent::p")

            let delivTerm2 =
                body.findElementWithoutException ("//b[contains(.,'Условия оплаты:')]/parent::p")

            let delivPlace =
                body.findElementWithoutException ("//b[contains(.,'Условия поставки:')]/parent::p")
