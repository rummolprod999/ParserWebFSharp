...
type ParserMedic(stn: Settings.T) =
    ...
    let url =
        "https://tender.medicina.ru/list.php"

    let timeoutB = TimeSpan.FromSeconds(60.)
    let listTenders = List<MedicRec>()
    ...
    member private __.Auth(driver: ChromeDriver) =
        ...
        wait.Until (fun dr ->
            dr
                .FindElement(
                    By.XPath("//input[@name = 'USER_LOGIN']")
                )
                .Displayed)
        |> ignore
        ...
        driver
            ...
            .SendKeys(Settings.UserMedic)

        driver
            .FindElement(By.XPath("//input[@name = 'USER_PASSWORD']"))
            .SendKeys(Settings.PassMedic)
    ...
    member private __.ParserSelen(driver: ChromeDriver) =
        ...
        driver
            .Navigate()
            .GoToUrl("https://tender.medicina.ru/list.php?login=yes&SHOWALL_1=1")
        ...
        wait.Until (fun dr ->
            dr
                .FindElement(
                    By.XPath(
                        "//table[@class = 'table table-striped table-hover table-condensed']/tbody/tr[position() > 1]"
                    )
                )
                .Displayed)
        |> ignore
    ...
    member private this.ParserTendersList (driver: ChromeDriver) (t: MedicRec) =
        try
            let T =
                TenderMedic(set, t, 263, "АО «Медицина»", "https://tender.medicina.ru/", driver)
            ...
            builder {
                ...
                let! purNum =
                    i.findElementWithoutException (
                        ".//span[contains(@class, 't_lot_id')]",
                        sprintf "purNum not found %s" i.Text
                    )

                let purNum =
                    purNum.Replace("Лот №:", "").Trim()
                ...
                let! endDateT =
                    i.findElementWithoutException ("./td[5]/span/nobr", sprintf "endDateT not found %s" i.Text)

                let! dateEnd = endDateT.DateFromString("dd.MM.yyyy HH:mm:ss", sprintf "endDate not parse %s" endDateT)

                let! pubDateT =
                    i.findElementWithoutException ("./td[4]/span/nobr", sprintf "pubDateT not found %s" i.Text)
                ...
                let cusName =
                    cusName.Replace("Компания:", "").Trim()

                let! person =
                    i.findElementWithoutExceptionOptional (
                        "./td[2]/div[@class = 't_lot_meta visible-xs']/span[contains(@class, 't_lot_responsible_fio')]",
                        sprintf "person not found %s" i.Text
                    )

                let person =
                    person.Replace("Ответственный:", "").Trim()

                let! phone =
                    i.findElementWithoutExceptionOptional (
                        "./td[2]/div[@class = 't_lot_meta visible-xs']/span[contains(@class, 't_lot_responsible_phone')]",
                        sprintf "phone not found %s" i.Text
                    )

                let phone =
                    phone.Replace("Телефон:", "").Trim()
