...
type TenderSibServ(stn : Settings.T, tn : SibServRec, typeFz : int, etpName : string, etpUrl : string) =
    ...
    override this.Parsing() =
        ...
        if reader.HasRows then reader.Close()
            ...
            reader.Close()
            let Page = Download.DownloadString1251 tn.Href
                ...
                | false -> 
                    ...
                    let addOrganizer =
                        sprintf 
                            ...
                            stn.Prefix
                    let contactPerson = ""
                    let postAddress = "125284, г. Москва, Ленинградский проспект, д. 31А, стр.1, 9 этаж"
                    let factAddress = ""
                    let phone = "8(495)225-75-95 доб. 182"
                    let inn = "0814118403"
            ...
            match updated with
            | true -> incr TenderSibServ.tenderUpCount
            | false -> incr TenderSibServ.tenderCount
            ...
            | _ -> 
                for doc in docs do
                    try 
                        ...
                        let docUrl = doc.GsnAtr ".//a" "href"
                        if docName <> "" && docUrl <> "" then 
                            let d =
                                { name = docName.Replace("скачать&nbsp;>>>", "").RegexCutWhitespace()
                                  url = docUrl }
                            tn.DocList.Add(d)
