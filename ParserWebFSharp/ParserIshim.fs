...
type ParserIshim(stn: Settings.T) =
    ...
    let url =
        "http://etp.ishim-agro.ru/?arrFilterLot_ff%5BARCHIVE_LOT%5D=Y&filter_for_submit=%D0%98%D1%81%D0%BA%D0%B0%D1%82%D1%8C+%D0%BB%D0%BE%D1%82%D1%8B&PAGEN_1=0&SORT_BY=DATE_START&SORT_ORDER=DESC"
    ...
    member private __.ParserSelen(driver: ChromeDriver) =
        ...
        driver
            .Navigate()
            .GoToUrl("http://etp.ishim-agro.ru/tenders_detail_noadm.php")
        ...
        wait.Until (fun dr ->
            dr
                .FindElement(
                    By.XPath("(//div[@class = 't_lot_info'])[10]")
                )
                .Displayed)
        |> ignore
    ...
    member private this.ParserTendersList (driver: ChromeDriver) (t: OsnovaRec) =
        try
            let T =
                TenderIshim(set, t, 331, "«ISHIM-AGRO»", "http://etp.ishim-agro.ru/", driver)
    ...
    member private __.Auth(driver: ChromeDriver) =
        ...
        driver
            ...
            .SendKeys(Settings.UserIshim)

        driver
            .FindElement(By.XPath("//input[@name = 'USER_PASSWORD']"))
            .SendKeys(Settings.PassIshim)
    ...
    member private this.ParserTenders(i: IWebElement) =
        ...
        let res =
            builder {
                ...
                let! purNum =
                    i.findElementWithoutException (".//span[@class = 't_lot_id']", sprintf "purNum not found %s" i.Text)

                let purNum = purNum.Replace("№", "")

                let! pubDateT =
                    i.findElementWithoutException (
                        ".//span[. = 'Начало:']/parent::span",
                ...
                let dateEnd =
                    endDateT.DateFromStringOrPubPlus2("dd.MM.yyyy HH:mm:ss", datePub)
